...
Docker Image คือ “ไฟล์ต้นแบบ (Template)” ที่ใช้สำหรับสร้าง Container
เปรียบเหมือน ไฟล์ .iso ของระบบปฏิบัติการ หรือ สูตรอาหาร

เป็นไฟล์แบบ read-only (แก้ไขโดยตรงไม่ได้)

เก็บทุกอย่างที่จำเป็น เช่น OS, Library, Runtime, Application

สามารถนำไปใช้สร้าง Container ได้หลายตัว
...
Docker Container คือ “อินสแตนซ์ที่กำลังรันอยู่” ของ Docker Image
เปรียบเหมือน เครื่องคอมพิวเตอร์ที่ติดตั้งจากไฟล์ .iso แล้วกำลังใช้งานอยู่

เป็นสิ่งที่รันจริง

มีสถานะ (running / stopped)

สามารถแก้ไขไฟล์ภายในได้ (แต่เมื่อ container ถูกลบ ข้อมูลที่ไม่ได้ mount volume จะหาย)
```
...
คำตอบ:
```
ในการทดลองมีการใช้ Named Volume สำหรับฐานข้อมูล เพื่อให้ข้อมูลไม่สูญหายเมื่อ container ถูกลบ และอาจใช้ Bind Mount สำหรับ Web Application เพื่อให้สามารถแก้ไขไฟล์จากเครื่อง host ได้ทันที ข้อดีคือช่วยให้ข้อมูล persistent และสะดวกต่อการพัฒนา แต่ข้อเสียคือ bind mount ผูกกับเครื่อง host และอาจเกิดปัญหาเมื่อย้ายระบบ
```
...
คำตอบ:
```
Network ใน Docker Compose ทำหน้าที่เป็น “เครือข่ายเสมือน (Virtual Network)”
ที่เชื่อมต่อ Container หลายตัวให้สื่อสารกันได้อย่างปลอดภัย

Container ภายในคุยกันได้
แต่คนภายนอกเข้าถึงได้เฉพาะ port ที่ expose เท่านั้น
...
คำตอบ:
```
depends_on ใช้กำหนดลำดับการเริ่มต้นของ container ใน Docker Compose เพื่อให้ service ที่พึ่งพากันเริ่มทำงานตามลำดับที่ถูกต้อง อย่างไรก็ตาม depends_on จะรอเพียงแค่ container เริ่มทำงาน (start) แต่ไม่ได้รอจน service พร้อมใช้งานจริง จึงควรใช้ healthcheck ร่วมด้วยเพื่อความถูกต้องของระบบ
```
...
คำตอบ:
```
การเปลี่ยนพอร์ตของ Moodle ใน Docker Compose ทำได้โดยแก้ไขค่าภายใต้ ports ใน service ของ Moodle โดยเปลี่ยนค่า Host Port เป็น 9000 เช่น "9000:8080" เพื่อให้สามารถเข้าใช้งานผ่าน http://localhost:9000
...
คำตอบ:
```
MOODLE_DB_HOST=db หมายถึงกำหนดให้ Moodle เชื่อมต่อฐานข้อมูลที่อยู่ใน container ชื่อ db ภายใน Docker network โดยไม่ใช้ localhost เพราะ localhost จะอ้างถึง container ตัวเอง ไม่ใช่ container อื่น Docker Compose มีระบบ DNS ภายในที่ทำให้ container สามารถสื่อสารกันผ่านชื่อ service ได้
```
...
คำตอบ:
```
การติดตั้ง Moodle ด้วย Docker มีข้อดีด้านความสะดวก ความรวดเร็ว และความสามารถในการควบคุม environment ให้เหมือนกันทุกเครื่อง เหมาะสำหรับงานทดลองและการพัฒนา ขณะที่การติดตั้งแบบปกติให้ความยืดหยุ่นและการควบคุมระบบที่ละเอียดกว่า เหมาะสำหรับระบบ Production ขนาดใหญ่ แต่มีขั้นตอนซับซ้อนและย้ายระบบได้ยากกว่า
```
...
คำตอบ:
```
หาก Container moodle_app ไม่สามารถเชื่อมต่อฐานข้อมูลได้ ควรตรวจสอบสถานะ container ด้วย docker ps ตรวจสอบ log ด้วย docker logs ตรวจสอบค่า environment variable โดยเฉพาะ MOODLE_DB_HOST ให้ตรงกับชื่อ service ของฐานข้อมูล และตรวจสอบ network ว่า container อยู่ใน network เดียวกัน รวมถึงตรวจสอบว่า database พร้อมใช้งานแล้วหรือไม่ โดยอาจใช้ healthcheck หรือ depends_on เพื่อแก้ไขปัญหา
```
...
คำตอบ:
```
คำสั่ง docker-compose down -v จะหยุดและลบ container ทั้งหมด พร้อมทั้งลบ volumes ที่เกี่ยวข้อง ทำให้ข้อมูลที่เก็บใน volume เช่น ฐานข้อมูล ถูกลบไปด้วย และเมื่อรันใหม่ ระบบจะเริ่มต้นเหมือนติดตั้งใหม่ทั้งหมด
