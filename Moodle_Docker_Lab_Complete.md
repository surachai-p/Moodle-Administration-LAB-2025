...
```
<img width="1006" height="172" alt="Screenshot 2026-02-06 134712" src="https://github.com/user-attachments/assets/9ad4d495-751e-40c9-81b2-818d3cebe93b" />
...
```
<img width="963" height="85" alt="Screenshot 2026-02-06 134817" src="https://github.com/user-attachments/assets/4530c5c9-6973-424e-ad06-072ffdf09b51" />
...
```
<img width="959" height="82" alt="Screenshot 2026-02-06 134836" src="https://github.com/user-attachments/assets/5d59ce6b-582b-4af9-a1b9-ae6c2c7b4eeb" />
...
```
<img width="970" height="312" alt="Screenshot 2026-02-06 134917" src="https://github.com/user-attachments/assets/ba89f383-456c-4eb4-97a4-0527d575f77a" />
...
```
<img width="1121" height="633" alt="Screenshot 2026-02-06 135441" src="https://github.com/user-attachments/assets/ae50614a-c231-4998-9304-cd4b7054b3ac" />
...
```bash
 รูปหน้า Dashboard 
```
<img width="1862" height="946" alt="Screenshot 2026-02-06 143137" src="https://github.com/user-attachments/assets/5ba15d67-ce48-499e-a9b4-d076726965bc" />
...
<img width="783" height="530" alt="Screenshot 2026-02-06 145548" src="https://github.com/user-attachments/assets/4646c185-7d7d-42eb-9de2-201a19ecc087" />

<img width="1880" height="863" alt="Screenshot 2026-02-06 153113" src="https://github.com/user-attachments/assets/afa57cac-2912-4704-9ffa-97fea39f5bdd" />

-ไม่สามารถส่งข้อมูลไปยังอีเมลล์ได้
...
```
<img width="1878" height="948" alt="Screenshot 2026-02-06 162729" src="https://github.com/user-attachments/assets/964d1a88-7de8-4b6e-bd43-8c6a13d2c272" />
...
```
<img width="1860" height="951" alt="Screenshot 2026-02-06 154148" src="https://github.com/user-attachments/assets/3f4bb85b-20a0-452c-81e4-0e4c34754ba2" />
...
```
<img width="1884" height="952" alt="Screenshot 2026-02-06 155541" src="https://github.com/user-attachments/assets/965488ca-479e-4a79-a008-ae44aa84fa44" />
...
```
<img width="1901" height="857" alt="Screenshot 2026-02-06 160002" src="https://github.com/user-attachments/assets/16988211-7a1f-4b50-b820-f629718643b7" />
...
```
<img width="1862" height="943" alt="Screenshot 2026-02-06 160311" src="https://github.com/user-attachments/assets/9adf6195-6386-40ac-837e-a23feca96865" />
<img width="1877" height="890" alt="Screenshot 2026-02-06 160924" src="https://github.com/user-attachments/assets/55343d17-6698-49ba-a9a9-852ebd1bdd7c" />
...
```
<img width="1878" height="906" alt="Screenshot 2026-02-06 162148" src="https://github.com/user-attachments/assets/bb188801-8bc7-4cd7-9926-605f083d253b" />
...
คำตอบ:
```
**Docker Image** คือ ต้นแบบหรือแม่พิมพ์ (Template) ของซอฟต์แวร์ที่ถูกผนึกรวมโค้ด, ไลบรารี และการตั้งค่าต่างๆ ไว้แล้ว มีสถานะเป็น Read-only (อ่านได้อย่างเดียว) แก้ไขไม่ได้ เปรียบเสมือนไฟล์ติดตั้งโปรแกรม (Setup file) หรือแผ่นเกม

**Docker Container** คือ อินสแตนซ์ที่ถูกสร้างขึ้นมาจาก Image เพื่อให้โปรแกรมทำงานจริง มีสถานะเป็น Read-write (อ่าน-เขียนได้) เปรียบเสมือนโปรแกรมที่ถูกติดตั้งและกำลังเปิดใช้งานอยู่บนเครื่อง

**ตัวอย่าง:**
- Image: `lthub/moodle:education-4.5.8` (ไฟล์ต้นฉบับที่ดาวน์โหลดมา)
- Container: `moodle_app` (ระบบ Moodle ที่กำลังรันอยู่และเราสามารถเข้าไปใช้งานได้)
```
...
คำตอบ:
```
มี Container ทั้งหมด **2 ตัว** ได้แก่:
1. **moodle_app** (สร้างจาก Image: lthub/moodle)
   - หน้าที่: เป็น Web Server (Apache) และรันตัวโปรแกรม Moodle (PHP) เพื่อให้บริการหน้าเว็บไซต์ LMS
2. **moodle_db** (สร้างจาก Image: mariadb)
   - หน้าที่: เป็น Database Server เก็บข้อมูลรายวิชา, ผู้ใช้งาน และการตั้งค่าต่างๆ ของระบบ
```
...
คำตอบ:
```
ในการทดลองใช้ **Named Volume** (เช่น `moodledata` และ `db_data`)

**ข้อดี:**
- ข้อมูลยังคงอยู่ (Persistence) แม้ลบ Container ทิ้งไปแล้ว
- แยกวงจรชีวิตของข้อมูลออกจากตัวแอปพลิเคชัน
- บริหารจัดการง่าย (Backup/Restore/Share) ผ่าน Docker CLI
- ปลอดภัยกว่าการเก็บข้อมูลไว้ใน Container โดยตรง

**ข้อเสีย:**
- ต้องใช้คำสั่ง Docker ในการเข้าถึงไฟล์ หรือต้องรัน Container เพื่อเข้าไปดูข้อมูล (เข้าถึงยากกว่า Bind Mount ที่เปิดดูผ่าน Folder ในเครื่องได้เลย)
```
...
คำตอบ:
```
**หน้าที่:** สร้างวงเครือข่ายจำลอง (Virtual Network) แบบ **Bridge Network** เพื่อให้ Container ภายในโปรเจกต์เดียวกันสามารถมองเห็นและสื่อสารกันได้ โดยแยกตัวออกจากเครือข่ายภายนอก (Isolation)

**การสื่อสาร:** Container สื่อสารกันโดยใช้ **Service Name (ชื่อบริการ)** เป็น Hostname แทน IP Address 
- ตัวอย่าง: Moodle เชื่อมต่อฐานข้อมูลโดยระบุ Host ว่า `db` (ตามชื่อ service ในไฟล์ docker-compose.yml) Docker จะแปลงชื่อ `db` เป็น IP Address ของ Container ฐานข้อมูลให้อัตโนมัติ
...
คำตอบ:
```
ใช้สำหรับ **กำหนดลำดับการเริ่มต้นทำงาน (Startup Order)** ของ Container เพื่อให้แน่ใจว่า Container ที่เป็นฐานข้อมูล (`db`) จะเริ่มทำงานก่อน Container ที่เป็นแอปพลิเคชัน (`moodle`) เนื่องจาก Moodle จำเป็นต้องเชื่อมต่อฐานข้อมูลทันทีที่เริ่มรัน หากฐานข้อมูลยังไม่พร้อม Moodle จะเกิด Error
```
...
คำตอบ:
```
แก้ไขในไฟล์ `docker-compose.yml` ภายใต้ service `moodle` ส่วนของ `ports:`
จากเดิม:
    ports:
      - "80:80"  (หรือ "8080:80")
เปลี่ยนเป็น:
    ports:
      - "9000:80"
(ความหมายคือ Map Port 9000 ของเครื่อง Host เข้ากับ Port 80 ของ Container)
...
คำตอบ:
```
หมายความว่า ให้ Moodle เชื่อมต่อไปยัง Server ฐานข้อมูลที่มีชื่อ Hostname ว่า **`db`** (ซึ่งเป็นชื่อ Service Name ของ MariaDB ในระบบ Docker Network นี้)

**เหตุผลที่ไม่ใช้ localhost:** เพราะ `localhost` ในบริบทของ Container จะหมายถึง **ตัว Container ของ Moodle เอง** ซึ่งไม่ได้รัน Database อยู่ (Database รันอยู่อีก Container หนึ่ง) ดังนั้นจึงต้องอ้างอิงชื่อเครื่องอื่นผ่านระบบ Network ภายใน
```
...
```
**ข้อดีของ Docker:**
- **ติดตั้งง่ายและรวดเร็ว:** รันคำสั่งเดียวได้ครบทั้ง Web Server, PHP, Database ไม่ต้องลงทีละตัว
- **Environment ตรงกัน:** ไม่ต้องกังวลเรื่องเวอร์ชัน PHP หรือ Library ไม่ตรงกัน (ลดปัญหา "It works on my machine")
- **ความสะอาด:** ไม่ทิ้งไฟล์ขยะไว้ในเครื่องหลัก ลบโปรเจกต์ทิ้งได้หมดจดเพียงลบ Container/Volume

**ข้อเสียของ Docker:**
- **ความซับซ้อนในการเรียนรู้:** ต้องมีความรู้เรื่อง Docker, Container, Network และ Volume
- **ประสิทธิภาพ:** อาจช้ากว่าการรันแบบ Native เล็กน้อย (โดยเฉพาะบน Windows/macOS ที่ต้องผ่าน Layer การจำลอง)
- **การแก้ไขไฟล์:** การเข้าไปแก้ไฟล์ Config ลึกๆ อาจยุ่งยากกว่าการแก้ไฟล์บนเครื่องโดยตรง
```
...
services:
  ...
  moodle:
    ...
    environment:
      # ... (Env เดิม) ...
      MOODLE_REDIS_HOST: redis  # เพิ่มตัวแปรเพื่อบอก Moodle ว่า Redis อยู่ที่ไหน
    depends_on:
      - db
      - redis  # เพิ่มให้รอ Redis รันก่อน
...
```
วิธีตรวจสอบ:
1. ใช้คำสั่ง `docker-compose ps` ดูว่า Container `moodle_db` มีสถานะ Up หรือไม่
2. ใช้คำสั่ง `docker-compose logs moodle` ดู Error Log ว่าแจ้งเตือนเรื่อง Connection refused หรือ Access denied
3. ตรวจสอบชื่อ Host ในไฟล์ Config ว่าใส่เป็น `db` หรือไม่

วิธีแก้ไข:
1. หาก Container DB ดับอยู่ ให้ใช้ `docker-compose up -d` หรือ `docker-compose restart db`
2. หากเป็นเรื่องรหัสผ่านไม่ตรง (Access denied) ให้ตรวจสอบไฟล์ `docker-compose.yml` ว่า `MYSQL_PASSWORD` กับ `MOODLE_DB_PASSWORD` ตรงกันทุกตัวอักษรหรือไม่
3. หากแก้รหัสผ่านแล้ว ให้รัน `docker-compose down -v` แล้ว up ใหม่ เพื่อล้างค่าเก่าที่จำไว้ใน Volume
```
...
คำตอบ:
```
Container ทั้งหมดจะถูกหยุดและลบออก **รวมถึง Named Volumes (db_data และ moodledata) ก็จะถูกลบออกไปด้วย** ผลที่ตามมาคือ **ข้อมูลทั้งหมดจะหายไปถาวร** (รายวิชาที่สร้าง, User ที่สมัคร, ข้อมูลในฐานข้อมูล) เหมือนกับการ Factory Reset ระบบใหม่
(คำสั่งปกติคือ `docker-compose down` เฉยๆ จะเก็บข้อมูลไว้ แต่ถ้ามี `-v` คือลบ Volume ด้วย)
