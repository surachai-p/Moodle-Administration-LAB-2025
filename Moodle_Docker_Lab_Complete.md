...
<img width="625" height="72" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 18 21 05" src="https://github.com/user-attachments/assets/21628a76-4ed7-4d73-ba6f-31b5e844e3bd" />
...
**บันทึกผลการทดลอง 2:**
 ...
 <img width="797" height="82" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 18 24 51" src="https://github.com/user-attachments/assets/20d2458b-1e68-4435-905e-f20a3ad1155e" />
...
<img width="787" height="99" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 18 25 15" src="https://github.com/user-attachments/assets/91396808-f21d-49ee-8cbb-e819b355b2c1" />
...
<img width="801" height="212" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 19 00 54" src="https://github.com/user-attachments/assets/eb962844-4ab1-4143-bd9e-06f5d9f8fb81" />
...
<img width="800" height="418" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 19 02 24" src="https://github.com/user-attachments/assets/4e88bf3a-f68d-4d82-adfe-f1a5c24557cf" />
...
<img width="1461" height="735" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 19 18 14" src="https://github.com/user-attachments/assets/115b2cac-d311-43cf-b2c2-5a224ae26477" />
...
<img width="1468" height="733" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 22 39 30" src="https://github.com/user-attachments/assets/45cdbea3-cab3-4a69-b07b-16cc0f2dd203" />
...
<img width="1469" height="831" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 20 06 25" src="https://github.com/user-attachments/assets/a212df0c-b013-4af5-930b-0beace6be657" />
...
<img width="1470" height="956" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 21 33 36" src="https://github.com/user-attachments/assets/e49c71db-8d1f-4e81-b778-ef24e85d0f72" />
...
<img width="885" height="924" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 21 39 58" src="https://github.com/user-attachments/assets/7b495008-1ca4-4768-8827-dc6da59731a9" />
...
<img width="883" height="925" alt="ภาพถ่ายหน้าจอ 2569-02-08 เวลา 21 46 43" src="https://github.com/user-attachments/assets/9189d3ff-9941-4327-b218-b1d6eb9e9b1a" />
...
<img width="1460" height="739" alt="ภาพถ่ายหน้าจอ2569-02-08เวลา 22 31 20" src="https://github.com/user-attachments/assets/f863233c-1961-4d7a-99a3-e53d02d86c1a" />
...
คำตอบ:
```
Docker Image คือ แม่แบบ (template) ที่บรรจุระบบปฏิบัติการ โปรแกรม และการตั้งค่าที่จำเป็น
Docker Container คือ instance ที่ถูกสร้างขึ้นจาก Image และกำลังทำงานอยู่

ตัวอย่าง:
- image: mariadb:latest คือ Docker Image
- moodle_db คือ Docker Container ที่รันจาก image mariadb
...
1) moodle_app
   - ทำหน้าที่รัน Moodle (PHP + Web Server)
   - ให้ผู้ใช้เข้าใช้งานผ่าน Web Browser

2) moodle_db
   - ทำหน้าที่เป็นฐานข้อมูล MariaDB
   - เก็บข้อมูลผู้ใช้ รายวิชา และกิจกรรมต่าง ๆ ของ Moodle
...
ข้อดี:
- ข้อมูลไม่หายเมื่อ Container ถูกลบหรือ restart
- จัดการง่าย ไม่ผูกกับ path บนเครื่อง host

ข้อเสีย:
- มองไม่เห็นไฟล์โดยตรงจากระบบ host
- ต้องใช้คำสั่ง Docker ในการจัดการข้อมูล
...
คำตอบ:
```
Docker Network ทำหน้าที่เป็นเครือข่ายภายในให้ Container ติดต่อกันได้

Container สื่อสารกันโดยใช้ชื่อ service เป็น hostname
เช่น moodle_app ติดต่อ database โดยใช้ชื่อ db แทน IP address
...
คำตอบ:
```
depends_on ใช้กำหนดลำดับการเริ่มต้นของ Container

เช่น moodle_app depends_on db
หมายความว่า Docker จะเริ่ม db ก่อน moodle_app
ช่วยลดปัญหาแอปพยายามเชื่อมต่อ database ก่อนที่ database จะพร้อม
...
คำตอบ:
```
แก้ไขที่ส่วน ports ของ service moodle_app
...
คำตอบ:
```
MOODLE_DB_HOST=db หมายถึง ให้ Moodle เชื่อมต่อฐานข้อมูลที่ service ชื่อว่า db

ไม่ใช้ localhost เพราะ:
- moodle_app และ db อยู่คนละ Container
- localhost จะชี้กลับมาที่ Container ตัวเอง
- Docker Network ใช้ชื่อ service ในการสื่อสารกัน
...
- ลบและติดตั้งใหม่ได้โดยไม่กระทบระบบหลัก

ข้อเสีย:
- ต้องเข้าใจ Docker เพิ่ม
- Debug บางกรณีซับซ้อนกว่า

ติดตั้งแบบปกติ:
ข้อดี:
- เหมาะกับผู้ที่ไม่ใช้ Docker
- ควบคุมระบบได้ละเอียด

ข้อเสีย:
- ติดตั้งยาก ใช้เวลานาน
- เสี่ยงชนกับโปรแกรมอื่นในเครื่อง
...
```
วิธีตรวจสอบ:
- ตรวจสอบ log ด้วย docker-compose logs moodle
- ตรวจสอบค่า environment variables ของ database
- ตรวจสอบว่า container db รันอยู่หรือไม่

วิธีแก้ไข:
- แก้ไข username / password ให้ตรงกัน
- ตรวจสอบ MOODLE_DB_HOST ให้ตรงกับชื่อ service
...
คำตอบ:
```
Container, Network และ Volume จะถูกลบทั้งหมด
ข้อมูลในฐานข้อมูลจะหายถาวร
เหมาะสำหรับการ reset ระบบ ไม่เหมาะกับระบบจริง
