...
คำตอบ:
Docker Image คือ ไฟล์ต้นแบบหรือแม่แบบของแอปพลิเคชัน ที่รวมโค้ด โปรแกรม และการตั้งค่าที่จำเป็น เช่น lthub/moodle หรือ mariadb
Docker Container คือ อินสแตนซ์ที่ถูกสร้างขึ้นจาก Image และกำลังทำงานอยู่ เช่น Container moodle_app ที่รัน Moodle หรือ moodle_db ที่รัน MariaDB

**2. จากสถาปัตยกรรมในการทดลอง มี Container กี่ตัว? แต่ละตัวมีหน้าที่อะไร?**

คำตอบ:
มี 2 Container
-moodle_app ทำหน้าที่เป็น Web Application Server สำหรับให้บริการ Moodle
-moodle_db ทำหน้าที่เป็น Database Server สำหรับเก็บข้อมูลผู้ใช้ เนื้อหา และข้อมูลระบบ

**3. จากการทดลองมีการจัดการ Volume แบบใด มีข้อดีข้อเสียอย่างไร?**

คำตอบ:
ใช้ Named Volume ได้แก่ db_data และ moodledata
ข้อดี:
...
-ใช้พื้นที่เก็บข้อมูลเพิ่มขึ้น

**4. Network ใน Docker Compose ทำหน้าที่อะไร? Container สื่อสารกันอย่างไร?**

คำตอบ:
Network ทำหน้าที่เชื่อมต่อ Container ให้สามารถสื่อสารกันได้ โดยใช้ Bridge Network
Container สามารถติดต่อกันผ่านชื่อ Service เช่น moodle_app ติดต่อ moodle_db ผ่าน hostname db


**5. `depends_on` ในไฟล์ docker-compose.yml มีความสำคัญอย่างไร?**

คำตอบ:
ใช้กำหนดลำดับการเริ่มต้น Container โดยให้ moodle เริ่มทำงานหลังจาก db เริ่มทำงานแล้ว เพื่อป้องกันปัญหาการเชื่อมต่อฐานข้อมูล

**6. ถ้าต้องการเปลี่ยน Port ของ Moodle  เป็น 9000 ต้องแก้ไขส่วนใดของไฟล์?**

คำตอบ:
แก้ไขในส่วน ports ของ service moodle เช่น
ports:
  - "9000:80"
...
คำตอบ:
หมายถึงให้ Moodle เชื่อมต่อฐานข้อมูลไปยัง Container ที่ชื่อ db
ไม่ใช้ localhost เพราะ Container แต่ละตัวทำงานแยกกัน localhost จะหมายถึง Container ตัวเอง ไม่ใช่ Container ฐานข้อมูล
...
-ติดตั้งรวดเร็ว
-ย้ายระบบได้ง่าย
-แยกส่วนประกอบของระบบชัดเจ
-ลดปัญหาความขัดแย้งของซอฟต์แวร์
ข้อเสีย:
-ต้องมีความรู้ Docker
-การแก้ไขปัญหาบางอย่างซับซ้อน
-ใช้ทรัพยากรเครื่องเพิ่มขึ้น
...
-ตรวจสอบ Logs ด้วย docker-compose logs moodle
...
-ตรวจสอบว่า db container ทำงานปกติ

**11. ถ้ารัน `docker-compose down -v` จะเกิดอะไรขึ้นกับข้อมูล?**

คำตอบ:
คำสั่งนี้จะหยุดและลบ Container รวมถึงลบ Volume ทั้งหมด ทำให้ข้อมูลในฐานข้อมูลและไฟล์ Moodle ถูกลบถาวร ไม่สามารถกู้คืนได้ หากไม่มีการสำรองข้อมูลไว้ก่อน
---
