...
คำตอบ: 
Docker Image คือพิมพ์เขียวหรือไฟล์ต้นฉบับที่แก้ไขไม่ได้เปรียบเสมือนตัวติดตั้งโปรแกรม ส่วน Docker Container คือตัวโปรแกรมที่รันขึ้นมาจาก Image นั้นและทำงานอยู่จริง เปรียบได้กับไฟล์ .exe เป็น Image และโปรแกรมที่กำลังเปิดใช้งานเป็น Container
```
...
คำตอบ:
ในการทดลองนี้มี 2 Container คือ moodle_app ทำหน้าที่เป็น Web Server สำหรับรันตัวระบบ Moodle และ moodle_db (MariaDB) ทำหน้าที่เป็นฐานข้อมูลสำหรับจัดเก็บข้อมูลทั้งหมด
```
...
คำตอบ:
มีการจัดการแบบ Named Volumes (เช่น moodledata, db_data) ซึ่งมีข้อดีคือข้อมูลไม่หายไปแม้จะหยุดหรือลบ Container แต่มีข้อเสียคือการเข้าไปจัดการหรือสำรองไฟล์โดยตรงจาก Host ทำได้ยากกว่าแบบ Bind Mount
```
...
คำตอบ:
Network ทำหน้าที่สร้างเครือข่ายเสมือนให้ Container เชื่อมต่อกัน โดยสื่อสารกันผ่านชื่อ Service (เช่น moodle_app คุยกับ db) ซึ่ง Docker จะทำการแปลงชื่อเป็น IP ภายในให้เองอัตโนมัติ
```
...
คำตอบ:
depends_on มีความสำคัญในการกำหนดลำดับการรัน โดยบอกให้ Moodle รอจนกว่า Database จะรันขึ้นมาพร้อมใช้งานก่อน เพื่อป้องกัน Error ในการเชื่อมต่อฐานข้อมูลขณะเริ่มต้นระบบ
```
...
คำตอบ:
ต้องแก้ไขในส่วนของ ports: โดยเปลี่ยนจาก "80:80" เป็น "9000:80" เพื่อให้เข้าใช้งานผ่าน localhost:9000 ได้
```
...
คำตอบ:
หมายความว่าให้ Moodle เชื่อมต่อฐานข้อมูลไปที่ Service ชื่อ db โดยไม่ใช้ localhost เพราะภายใน Container นั้น localhost จะหมายถึงตัวมันเอง ไม่ใช่เครื่อง Host หรือ Container อื่น
```
...
คำตอบ:
ข้อดีของ Docker คือติดตั้งง่าย ย้ายเครื่องได้สะดวก และจัดการเวอร์ชันได้เป๊ะ แต่ข้อเสียคือต้องกินทรัพยากรเครื่องมากกว่าการติดตั้งปกติเล็กน้อยและมีความซับซ้อนในการตั้งค่า Network
```
...
คำตอบ (เขียน YAML):
การเพิ่ม Redis ต้องเพิ่ม Service ใหม่ภายใต้ services: โดยระบุ image: redis:latest และเพิ่ม links หรือ network ให้เชื่อมกับ moodle_app
```yaml
...
```
วิธีตรวจสอบ:
วิธีตรวจสอบคือใช้คำสั่ง docker logs moodle_app เพื่อดู Error และเช็กสถานะด้วย docker ps


วิธีแก้ไข:
แก้ไขคือตรวจสอบรหัสผ่านใน Environment Variables ว่าตรงกับฐานข้อมูลหรือไม่ หรือเช็กว่า Database รันอยู่จริงหรือไม่
...
คำตอบ:
ข้อมูลทั้งหมดจะถูกลบถาวร รวมถึงข้อมูลในฐานข้อมูลและไฟล์ที่อัปโหลด (Volumes) เนื่องจากออปชัน -v จะทำการสั่งลบ Volume ที่ผูกอยู่กับ Container ทิ้งไปทั้งหมด
```
