...
```
<img width="1122" height="437" alt="image" src="https://github.com/user-attachments/assets/4ade6c97-647f-4aa2-9230-0ff38c1fe3da" />
...
```
<img width="1417" height="302" alt="image" src="https://github.com/user-attachments/assets/a7e29868-7553-4b5b-9510-9ddfb8764476" />
...
```
<img width="1870" height="280" alt="image" src="https://github.com/user-attachments/assets/47c5119a-d623-4dec-bea1-538e77df8310" />
...
```
<img width="1921" height="942" alt="image" src="https://github.com/user-attachments/assets/03d1330e-87d9-4958-802a-6b59f60cf741" />
...
```
<img width="1616" height="738" alt="image" src="https://github.com/user-attachments/assets/3eec80df-aacd-44c1-9cb9-a3b1f1dce858" />
...
```bash
 รูปหน้า Dashboard 
```
<img width="2255" height="1503" alt="image" src="https://github.com/user-attachments/assets/6f75e516-11a0-4f15-ad17-d6b0151d9a06" />
...
```
<img width="2252" height="1498" alt="image" src="https://github.com/user-attachments/assets/f48ece93-02db-4d9b-a014-db778280b49b" />
...
```
<img width="2255" height="1245" alt="image" src="https://github.com/user-attachments/assets/3723c82e-d099-4c91-b627-87b27df4eadd" />
...
```
<img width="2255" height="1274" alt="image" src="https://github.com/user-attachments/assets/e33a8e04-773b-47c3-a0ed-eac7e664c4a4" />
...
```
<img width="2254" height="1277" alt="image" src="https://github.com/user-attachments/assets/3f724de0-ada8-46af-ad94-36cc904de200" />
...
```
<img width="2255" height="1284" alt="image" src="https://github.com/user-attachments/assets/d71d8cac-0f9d-4af4-9e06-5f4901d09462" />
...
```
<img width="2208" height="1255" alt="image" src="https://github.com/user-attachments/assets/991d7745-04a0-4a12-b917-94ee0909e1f5" />
...
คำตอบ:
```
Docker Image คือแม่แบบ (Template) ที่ใช้สร้าง Container ภายใน Image จะประกอบด้วยระบบปฏิบัติการ ไลบรารี และซอฟต์แวร์ที่จำเป็น

Docker Container คือ instance ที่ถูกรันขึ้นมาจาก Image เป็นสภาพแวดล้อมที่พร้อมใช้งานจริง
```
...
1.moodle_app
-ทำหน้าที่เป็น Web Application
-รัน Moodle และ Apache
2.moodle_db
-ทำหน้าที่เป็น Database Server
-ใช้ MariaDB สำหรับเก็บข้อมูลของ Moodle
```
...
คำตอบ:
```
ใช้ Named Volume ใน Docker
ข้อดี
-ข้อมูลไม่หายเมื่อ Container ถูกลบหรือ Restart
-แยกข้อมูลออกจากตัว Container อย่างชัดเจน
-สำรองข้อมูลได้ง่าย
ข้อเสีย
...
คำตอบ:
```
Docker Compose จะสร้าง Network อัตโนมัติ เพื่อให้ Container ในไฟล์เดียวกันสามารถสื่อสารกันได้
Container จะสื่อสารกันผ่าน ชื่อ service (เช่น db)
...
เช่น ให้ moodle_app เริ่มทำงานหลังจาก moodle_db ถูกสร้างแล้ว
ช่วยลดปัญหา Application รันก่อน Database พร้อมใช้งาน
```
...
คำตอบ:
```
MOODLE_DB_HOST=db หมายถึง ให้ Moodle เชื่อมต่อ Database ที่ service ชื่อ db ที่ไม่ใช้ localhost เพราะ localhost จะอ้างถึง Container ตัวเอง ไม่ใช่ Container Database
```
...
ข้อเสีย:-ต้องเข้าใจ Docker เพิ่ม
-Debug บางกรณีซับซ้อน

ติดตั้งแบบปกติ
ข้อดี:-ควบคุมระบบได้ละเอียด
-เข้าใจโครงสร้างชัดเจน
ข้อเสีย:-ติดตั้งยุ่งยาก
-เสี่ยงต่อปัญหา Dependency
```
...
```
วิธีตรวจสอบ:docker ps


วิธีแก้ไข:docker logs moodle_app
...
คำตอบ:
```
Container จะถูกหยุดและลบ
Volume ทั้งหมดจะถูกลบ
ข้อมูลใน Database และไฟล์ถาวรจะ สูญหายทั้งหมด
```
