...
คำตอบ:
```
Docker Image: เป็นเทมเพลตต้นแบบที่อ่านได้อย่างเดียว (Read-only) บรรจุซอฟต์แวร์และการตั้งค่าพื้นฐานไว้ เปรียบเหมือนแผ่นซีดีติดตั้งเกม
Docker Container: เป็นตัวงานที่รันขึ้นมาจาก Image สามารถอ่านและเขียนข้อมูลได้ เปรียบเหมือนตัวเกมที่ติดตั้งและกำลังเล่นอยู่
```
...
```
มีทั้งหมด 2 ตัว
1. moodle_app: ทำหน้าที่เป็น Web Server และประมวลผล PHP ของระบบ Moodle 
2. moodle_db: ทำหน้าที่เป็น Database Server (MariaDB) สำหรับเก็บข้อมูลฐานข้อมูลทั้งหมด
```
...
คำตอบ:
```
จัดการแบบ Named Volume ครับ 
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container และสามารถแชร์ข้อมูลระหว่าง Container ได้ง่าย 
ข้อเสีย: ข้อมูลถูกจัดการโดย Docker ทำให้การเข้าไปดูไฟล์โดยตรงจากเครื่อง Host ทำได้ยากกว่าแบบอื่น
```
...
คำตอบ:
```
หน้าที่: สร้างเครือข่ายเสมือนเพื่อให้ Container ต่าง ๆ สื่อสารกันได้ภายในกลุ่ม 
การสื่อสาร: Container สื่อสารกันผ่าน Container name (เช่นระบุชื่อ db แทนเลข IP)
```
...
คำตอบ:
```
  ใช้กำหนด ลำดับการเริ่มทำงาน ของ Container เช่น สั่งให้ฐานข้อมูล (db) เริ่มทำงานก่อนตัวแอป (moodle) เพื่อป้องกันแอปพังเพราะหาฐานข้อมูลไม่เจอ
```
...
คำตอบ:
```
แก้ไขที่หัวข้อ ports: ของ service moodle ในไฟล์ docker-compose.yml 
จากเดิม "80:80" ให้เปลี่ยนเป็น "9000:80"
```
...
คำตอบ:
```
ความหมาย: เป็นการบอก Moodle ให้เชื่อมต่อฐานข้อมูลไปที่ Container ที่ชื่อว่า db 
เหตุผลไม่ใช้ localhost เพราะแต่ละ Container แยก Network namespace กัน หากใช้ localhost ตัว Moodle จะมองหาฐานข้อมูลภายในตัวเอง ไม่ได้มองไปที่ Container db
```
...
คำตอบ:
```
ข้อดี: ติดตั้งเร็ว ย้ายเครื่องง่าย และสภาพแวดล้อม (Environment) เหมือนกันทุกเครื่อง 
ข้อเสีย: ต้องมีความรู้เรื่อง Docker และอาจใช้ทรัพยากรเครื่องมากกว่าการติดตั้งแบบ Native เล็กน้อย
```
...
คำตอบ:
```
  วิธีตรวจสอบ: ใช้คำสั่ง docker-compose ps ดูสถานะว่า db รันอยู่หรือไม่ หรือใช้ docker-compose logs moodle ดูข้อผิดพลาด
  วิธีแก้ไข: ตรวจสอบการตั้งค่า Environment Variables ในไฟล์ YAML ให้รหัสผ่านตรงกัน หรือสั่ง docker-compose restart
...
คำตอบ:
```
  Container จะถูกลบ และ ข้อมูลทั้งหมดใน Volumes จะถูกลบถาวร (All data lost)
