...
<img width="930" height="677" alt="image" src="https://github.com/user-attachments/assets/bf1b598d-0ccd-4e95-9378-00b448230b83" />
<img width="912" height="140" alt="image" src="https://github.com/user-attachments/assets/fcac9015-9c57-419d-8852-7821f3b2374c" />
...
**บันทึกผลการทดลอง 10:**
```bash
 <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/3f63348e-0feb-4105-a87c-06e3aba62e34" />
...
5. 
   **บันทึกผลการทดลอง 11:**
```bash
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/9d0d1c73-635f-49d8-9a58-a0554d03b0bd" />
...
คำตอบ:Image คือต้นฉบับที่เป็นเหมือนพิมพ์เขียวส่วน Container คือร่างที่ทำงานจริงจาก Image นั้น เช่น Image คือตัวติดตั้ง Moodle และ Container คือหน้าเว็บ Moodle ที่กำลังรันอยู่
```
...
คำตอบ:มี 2 ตัว ได้แก่ moodle ทำหน้าที่เป็น Web Server ประมวลผลหน้าเว็บ และ db ทำหน้าที่เป็น Database Server เก็บข้อมูลทั้งหมดของระบบ
```
...
คำตอบ:ใช้แบบ Named Volume ข้อดีคือข้อมูลไม่หายแม้ลบ Container และจัดการง่ายผ่าน Docker ข้อเสียคือไฟล์ถูกเก็บในพื้นที่ของ Docker ทำให้เข้าถึงจาก Host โดยตรงได้ยาก
```
...
คำตอบ:สร้างเครือข่ายเสมือนให้ Container รู้จักกัน โดยสื่อสารกันผ่านชื่อ Service Name
```
...
คำตอบ:กำหนดลำดับการเริ่มทำงาน เพื่อให้มั่นใจว่า Database จะรันขึ้นมาพร้อมใช้งานก่อนที่ Moodle จะเริ่มทำงาน เพื่อป้องกันข้อผิดพลาดในการเชื่อมต่อ
```
...
คำตอบ:แก้ไขที่ส่วน ports ใน Service Moodle โดยเปลี่ยนตัวเลขหน้าเครื่องหมาย colon
```
...
คำตอบ:หมายถึงการชี้เป้าให้ Moodle ไปหาฐานข้อมูลที่ชื่อ db ที่ไม่ใช้ localhost เพราะในระบบ Container localhost จะหมายถึงตัว Container ไม่ใช่เครื่องคอมพิวเตอร์หลัก
```
...
คำตอบ:ข้อดี คือติดตั้งเร็ว ย้ายเครื่องง่ายและสภาพแวดล้อมไม่เพี้ยน ข้อเสีย คือต้องมีความรู้เรื่องคำสั่ง Docker และใช้ทรัพยากรเครื่องมากกว่าการลงแบบปกติเล็กน้อย
```
...
คำตอบ:
```
วิธีตรวจสอบ:ใช้คำสั่ง docker-compose logs moodle เพื่อดูข้อความ Error หรือเช็คสถานะด้วย docker-compose ps


วิธีแก้ไข:ตรวจสอบชื่อ Host, Port และรหัสผ่านใน environment ให้ตรงกันทั้งฝั่ง Moodle และ DB

```

**11. ถ้ารัน `docker-compose down -v` จะเกิดอะไรขึ้นกับข้อมูล?**

คำตอบ:ข้อมูลทั้งหมดรวมถึงฐานข้อมูลที่อยู่ใน Volume จะถูกลบทิ้งถาวร ไม่สามารถกู้คืนได้
```
