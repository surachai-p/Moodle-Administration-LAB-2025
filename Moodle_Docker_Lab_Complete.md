...
```
<img width="785" height="302" alt="image" src="https://github.com/user-attachments/assets/01dda2d6-0517-47db-a153-8bbbe3ef0816" />
...
```
<img width="1184" height="179" alt="image" src="https://github.com/user-attachments/assets/82b144e7-ac4f-4138-be4d-e90511940439" />
...
```
<img width="1502" height="114" alt="image" src="https://github.com/user-attachments/assets/a0bed635-35af-4e16-b4dd-b96ecade1cb9" />
...
```
<img width="1552" height="111" alt="image" src="https://github.com/user-attachments/assets/b2ab32bf-dea2-4400-899d-5e3dca8f0881" />
...
```
<img width="1073" height="589" alt="image" src="https://github.com/user-attachments/assets/1b2497cd-909c-42ed-93b3-87a3f2fa01f6" />
...
```bash
 รูปหน้า Dashboard 
```
<img width="1915" height="1135" alt="image" src="https://github.com/user-attachments/assets/f635dfdb-95af-4354-8891-617c6e7c6523" />
...
```
<img width="1919" height="1139" alt="image" src="https://github.com/user-attachments/assets/79aedc86-bf05-4240-b134-00051080a768" />
...
```
<img width="938" height="1072" alt="image" src="https://github.com/user-attachments/assets/47517f7c-498e-4be6-83ef-54ef9160bdca" />
...
```
<img width="1918" height="1133" alt="image" src="https://github.com/user-attachments/assets/69e64d79-2a84-424e-964b-3620a6e7a82b" />
...
```
<img width="1781" height="828" alt="image" src="https://github.com/user-attachments/assets/8d4d9288-99f9-4148-a3d2-63ba2ae9dc0b" />
...
```
<img width="1776" height="932" alt="image" src="https://github.com/user-attachments/assets/cca8d3f7-055b-4f8e-adec-6dfd63ddbf5c" />
...
```
<img width="1784" height="946" alt="image" src="https://github.com/user-attachments/assets/09ae1f7a-9c1b-42d0-bb55-7f199f18251d" />
...
คำตอบ:
```
Image คือไฟล์ต้นฉบับที่เป็น "พิมพ์เขียว" (Template) แบบ Read-only ที่รวมโปรแกรมและสภาพแวดล้อมไว้ ส่วน Docker Container คือสิ่งที่รันขึ้นมาจาก Image เพื่อใช้งานจริง (Instance) สามารถเขียนข้อมูลทับได้

ตัวอย่าง: Image เปรียบเสมือน แผ่นโปรแกรมติดตั้ง Moodle ส่วน Container เปรียบเสมือน เว็บไซต์ Moodle ที่กำลังทำงานอยู่
```
...
คำตอบ:
```
moodle_app: ทำหน้าที่เป็น Web Server (เช่น Apache/Nginx) และประมวลผลโค้ด PHP ของ Moodle

moodle_db: ทำหน้าที่เป็น Database Server (เช่น MySQL/MariaDB) สำหรับจัดเก็บฐานข้อมูลตัวอักษรทั้งหมด
```
...
คำตอบ:
```
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container (Persistence) และ Docker จัดการเรื่องสิทธิ์การเข้าถึงไฟล์ให้โดยอัตโนมัติ

ข้อเสีย: เข้าถึงไฟล์ได้ยากจากหน้า Windows Explorer โดยตรง เพราะถูกเก็บไว้ในพื้นที่ภายในของ Docker Desktop
```
...
คำตอบ:
```
ทำหน้าที่สร้าง เครือข่ายเสมือน (Isolated Virtual Network) เพื่อให้ Container ที่อยู่ในกลุ่มเดียวกันคุยกันได้ โดยสื่อสารกันผ่าน Service Name (เช่น db) ซึ่ง Docker จะมีระบบ DNS ภายในช่วยค้นหา IP Address ให้เอง
...
คำตอบ:
```
ใช้กำหนด ลำดับการเริ่มทำงาน (Startup Order) เพื่อให้มั่นใจว่า Container ฐานข้อมูลจะถูกรันขึ้นมาเตรียมพร้อมก่อนที่ตัว Moodle จะเริ่มทำงาน เพื่อป้องกันข้อผิดพลาดในการเชื่อมต่อฐานข้อมูล
```
...
คำตอบ:
```
แก้ไขที่ส่วน ports: ของ Service moodle_app โดยเปลี่ยนจาก 80:80 เป็น 9000:80
...
คำตอบ:
```
หมายความว่าให้ Moodle เชื่อมต่อไปยังฐานข้อมูลที่ชื่อ Service ว่า db สาเหตุที่ไม่ใช้ localhost เพราะในโลกของ Docker คำว่า localhost ภายใน Container จะหมายถึงตัวมันเองเท่านั้น ไม่ได้หมายถึงเครื่อง Host หรือ Container อื่น
```
...
คำตอบ:
```
ข้อดี: ติดตั้งและย้ายเครื่องได้รวดเร็ว (Portability), ควบคุมเวอร์ชันของ PHP/DB ได้แม่นยำ, ไม่ทำให้ซอฟต์แวร์ในเครื่องโฮสต์รก

ข้อเสีย: ต้องใช้ทรัพยากรเครื่องมากกว่าปกติเล็กน้อย (Overhead), ต้องมีความรู้เฉพาะทางด้านคำสั่ง Docker
```
...
คำตอบ:
```
วิธีตรวจสอบ: ใช้คำสั่ง docker logs moodle_app เพื่อดูข้อความแจ้งเตือนความผิดพลาด และใช้ docker ps ดูว่า Container ฐานข้อมูลรันอยู่หรือไม่


วิธีแก้ไข: ตรวจสอบว่า DB_HOST, DB_USER, DB_PASSWORD ในไฟล์ docker-compose.yml ตรงกันทั้งสองฝั่ง และดูว่ารันอยู่ใน networks เดียวกันหรือไม่
...
คำตอบ:
```
ข้อมูลทั้งหมดจะถูกลบทิ้งถาวร เนื่องจาก Flag -v จะสั่งให้ Docker ลบทั้ง Container และ Named Volumes (ฐานข้อมูลและไฟล์ที่อัปโหลดทั้งหมด) ทิ้งไปพร้อมกัน
