...
<img width="1919" height="991" alt="image" src="https://github.com/user-attachments/assets/6a5644c9-5c07-427d-9671-148dbe8306a5" />
...
<img width="1277" height="182" alt="image" src="https://github.com/user-attachments/assets/f7560cdd-c77a-43fe-a64f-c40332fc0c8e" />
...
<img width="1203" height="88" alt="image" src="https://github.com/user-attachments/assets/2fcf56ee-decd-4ea1-a9b4-123859f363cf" />
...
<img width="1347" height="538" alt="image" src="https://github.com/user-attachments/assets/a78d6c8b-db56-4226-a7cb-b1e4dab96a76" />
...
<img width="923" height="330" alt="image" src="https://github.com/user-attachments/assets/d708ef7c-48cc-482b-b906-279535d0ffcf" />
...
<img width="1899" height="909" alt="image" src="https://github.com/user-attachments/assets/8f0a3236-059f-4094-84b2-7ee14ab3238b" />
...
No-reply address: your-email@kmitl.ac.th
```
...
<img width="1901" height="967" alt="image" src="https://github.com/user-attachments/assets/76953be0-8145-4087-a448-3fd5d1a9bded" />
...
<img width="1919" height="874" alt="image" src="https://github.com/user-attachments/assets/0eff9889-e22f-4b2d-b8fb-195d3e91ef58" />
...
<img width="1751" height="563" alt="image" src="https://github.com/user-attachments/assets/ad34d141-6184-489e-930a-67fe03ecb88b" />
<img width="1919" height="910" alt="image" src="https://github.com/user-attachments/assets/52b5afc6-e253-4ded-952c-905317ffd7e4" />
...
<img width="1919" height="789" alt="image" src="https://github.com/user-attachments/assets/fa3cd1b6-675b-461f-9ade-1dff076ac0c2" />
...
<img width="1897" height="906" alt="image" src="https://github.com/user-attachments/assets/d28f410c-7def-4158-9ca4-44e9491fe8da" />
...
<img width="1919" height="900" alt="image" src="https://github.com/user-attachments/assets/91473858-1db8-489c-948b-16bf5f052fff" />
...
คำตอบ:
```
Docker Image คือ "ต้นแบบ" หรือไฟล์ Read-only ที่รวมโปรแกรมและ Environment ทุกอย่างไว้ (เหมือนพิมพ์เขียว) ส่วน Docker Container คือ "ร่างจำลอง" ที่ถูกรันขึ้นมาจาก Image เพื่อใช้งานจริง (เหมือนบ้านที่สร้างเสร็จแล้ว)
ตัวอย่าง: Image เปรียบเสมือนไฟล์ติดตั้งโปรแกรม (.exe) ส่วน Container เปรียบเสมือนตอนที่เราดับเบิลคลิกเปิดโปรแกรมนั้นขึ้นมาทำงาน
```
...
```
มี 2 ตัวหลัก คือ:
1. moodle: ทำหน้าที่เป็น Web Server ประมวลผลภาษา PHP และให้บริการตัวเว็บไซต์ Moodle
2. mariadb (หรือ db): ทำหน้าที่เป็น Database Server เก็บข้อมูลทุกอย่าง เช่น รายชื่อผู้ใช้, รายวิชา และคะแนน
```
...
คำตอบ:
```
ใช้แบบ Named Volumes (เช่น moodle_data, mariadb_data)
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container, Docker จัดการตำแหน่งเก็บไฟล์ให้โดยอัตโนมัติ, มีประสิทธิภาพการเขียนอ่านสูง
ข้อเสีย: เข้าถึงไฟล์โดยตรงจากหน้า Windows/Host ได้ยาก (ต้องใช้คำสั่ง Docker หรือเข้าไปดูใน Docker Path)
```
...
คำตอบ:
```
ทำหน้าที่สร้างเครือข่ายจำลอง (Virtual Network) เพื่อให้ Container ในกลุ่มเดียวกันมองเห็นกันได้
Container สื่อสารกันโดยใช้ "Service Name" (เช่น moodle เชื่อมต่อไปยัง db) โดย Docker จะมีระบบ DNS ภายในคอยแปลงชื่อ service เป็น IP Address ให้เอง
```
...
คำตอบ:
```
ใช้กำหนดลำดับการเริ่มทำงานของ Container เพื่อให้แน่ใจว่าตัวฐานข้อมูล (Database) จะต้องถูกสร้างและทำงาน (Ready) ก่อนที่ตัว Moodle จะเริ่มรันขึ้นมาเชื่อมต่อ
```
...
คำตอบ:
```
แก้ไขที่หัวข้อ ports ของ service moodle จากเดิม "80:8080" ให้เป็น "9000:8080"
(เลขซ้ายคือ Port ภายนอกที่เราเรียกผ่าน Browser, เลขขวาคือ Port ภายใน Container)
```
...
คำตอบ:
```
หมายถึงการตั้งค่าให้ Moodle เชื่อมต่อไปยังฐานข้อมูลที่ชื่อ Service ว่า "db" 
ที่ไม่ใช้ localhost เพราะในระบบ Container คำว่า localhost จะหมายถึง "ตัวเอง" ซึ่ง Moodle ไม่ได้เก็บฐานข้อมูลไว้ใน Container เดียวกัน จึงต้องเรียกผ่านชื่อ Service ในระบบ Network แทน
```
...
คำตอบ:
```
ข้อดี: ติดตั้งง่ายและเร็วมาก, ย้ายไปเครื่องไหนก็ทำงานได้เหมือนเดิม (Portable), ไม่ตีกับโปรแกรมอื่นในเครื่อง
ข้อเสีย: สิ้นเปลืองทรัพยากร (RAM/Storage) มากกว่าเล็กน้อย, ต้องมีความรู้พื้นฐานคำสั่ง Docker ในการดูแลรักษา
```
...
คำตอบ:
```
วิธีตรวจสอบ: ใช้คำสั่ง docker-compose logs เพื่อดู Error Message และใช้ docker ps ดูว่าตัว DB รันอยู่หรือไม่

วิธีแก้ไข: ตรวจสอบการตั้งค่า Environment Variables ในไฟล์ YAML ว่า username/password และชื่อ host ตรงกันหรือไม่
```
...
คำตอบ:
```
Container จะถูกหยุดและลบทิ้ง พร้อมกับลบ "Volume" (ข้อมูลในฐานข้อมูลและไฟล์ที่อัปโหลดทั้งหมด) ทิ้งไปอย่างถาวร ไม่สามารถกู้คืนได้
```
