...
<img width="944" height="286" alt="image" src="https://github.com/user-attachments/assets/b213a7f6-2303-4089-ad59-2e107134b6ad" />
...
bash
 ...
 <img width="969" height="196" alt="image" src="https://github.com/user-attachments/assets/c7a98e27-1a3b-4637-83de-1602bead27e2" />
...
bash
 ...
 <img width="956" height="199" alt="image" src="https://github.com/user-attachments/assets/46fdff8c-4cf9-4bcf-abd7-815b59f641cb" />
...
bash
 ...
 <img width="940" height="278" alt="image" src="https://github.com/user-attachments/assets/7f1f3aac-1ae1-4718-a6f5-30d03324fd71" />
...
bash
 ...
 <img width="938" height="693" alt="image" src="https://github.com/user-attachments/assets/e68c2da4-8a6e-488c-9209-e03e1a39d697" />
...
bash
 รูปหน้า Dashboard 
```
<img width="1875" height="907" alt="image" src="https://github.com/user-attachments/assets/6e8c60b2-5582-4a5b-b050-e93328266908" />
...
```
<img width="1772" height="861" alt="image" src="https://github.com/user-attachments/assets/3daac998-bb4e-41ff-80ec-0e58c380c47d" />
...
<img width="1747" height="691" alt="image" src="https://github.com/user-attachments/assets/c5824998-e1ab-4e42-95bf-84ca3053b07e" />
...
<img width="1855" height="837" alt="image" src="https://github.com/user-attachments/assets/b7c072a2-ebf0-484a-97d2-ad59a8f69005" />
<img width="1064" height="792" alt="image" src="https://github.com/user-attachments/assets/c4a31e5d-8b4c-4141-b497-adc28e175ada" />
...
<img width="1644" height="272" alt="image" src="https://github.com/user-attachments/assets/478bdc3e-c947-473a-b6c2-75956213a488" />
...
<img width="1850" height="619" alt="image" src="https://github.com/user-attachments/assets/1e158f80-9ea1-4707-a77d-2b2298b2189d" />
...
<img width="1637" height="706" alt="image" src="https://github.com/user-attachments/assets/04673c38-e572-4ab6-8365-3edf96049db3" />
...
คำตอบ: Docker Image: เปรียบเสมือน "พิมพ์เขียว" หรือแผ่นซีดีติดตั้งโปรแกรม เป็นไฟล์ที่อ่านได้อย่างเดียว (Read-only) ที่รวมทุกอย่างที่โปรแกรมต้องใช้ไว้

Docker Container: คือ "ร่างจำลอง" ที่ทำงานอยู่จริงจากการรัน Image เปรียบได้กับโปรแกรมที่กำลังทำงานอยู่ในหน่วยความจำ
```
...
คำตอบ: มี 2 ตัวหลัก:
moodle_app: ทำหน้าที่เป็น Web Server ประมวลผลภาษา PHP และให้บริการหน้าเว็บ Moodle

db (MariaDB): ทำหน้าที่เป็นฐานข้อมูล เก็บข้อมูลผู้ใช้ คอร์สเรียน และคะแนนต่างๆ
```
...
คำตอบ:แบบ Named Volumes: (เช่น moodle_data, mariadb_data)

ข้อดี: ข้อมูลไม่หายเมื่อลบ Container, จัดการง่ายผ่าน Docker และมีประสิทธิภาพการอ่านเขียนสูง

ข้อเสีย: เข้าถึงไฟล์ได้ยากกว่าแบบ Bind Mount (เพราะ Docker เก็บไว้ในพื้นที่จัดการของมันเอง)
```
...
คำตอบ:ทำหน้าที่: สร้างวงเครือข่ายเสมือนเพื่อให้ Container คุยกันเองได้

การสื่อสาร: สื่อสารผ่าน Service Name (เช่น Moodle จะมองหาฐานข้อมูลผ่านชื่อ db แทนการใช้ IP Address)
```
...
คำตอบ:ใช้กำหนดลำดับการรัน Container เช่น ให้ db เริ่มทำงานและพร้อมใช้งานก่อนที่ moodle จะเริ่มทำงาน เพื่อป้องกัน Error จากการหาฐานข้อมูลไม่เจอ
```
...
คำตอบ:แก้ไขที่ส่วน ports: ใน moodle_app จากเดิม 80:8080 เป็น 9000:8080
```
...
คำตอบ: ให้ Moodle เชื่อมต่อไปยังเครื่องที่ชื่อว่า db (ซึ่งคือ Container ฐานข้อมูล)
ไม่ใช้ localhost: เพราะในโลกของ Docker แต่ละ Container คือเครื่องจำลองแยกกัน localhost ของ Moodle จะหมายถึงตัวมันเอง ไม่ใช่เครื่องฐานข้อมูลครับ
```
...
คำตอบ: ข้อดี: ติดตั้งเร็วมาก (คลิกเดียวจบ), ย้ายเครื่องง่าย (Portable), เวอร์ชั่นของ PHP/DB ไม่ตีกับงานอื่นในเครื่อง

ข้อเสีย: ต้องเรียนรู้คำสั่ง Docker เพิ่มเติม และกินทรัพยากรเครื่องมากกว่าการลงแบบ Native เล็กน้อย
```
...
คำตอบ:วิธีตรวจสอบ: ใช้คำสั่ง docker-compose logs moodle_app เพื่อดู Error หรือเช็คสถานะด้วย docker ps

วิธีแก้ไข: ตรวจสอบว่า db รันอยู่หรือไม่, เช็ค Username/Password ในไฟล์ YAML ว่าตรงกันหรือไม่
```
วิธีตรวจสอบ:
...
คำตอบ:ข้อมูลหาย เพราะ Option -v จะไปลบ Volumes ทิ้งด้วย ทำให้ข้อมูลเว็บไซต์และฐานข้อมูลที่คุณสร้างไว้ถูกลบทิ้งถาวร
```
