...
``
 ...
 <img width="736" height="558" alt="image" src="https://github.com/user-attachments/assets/6103e3b7-59dc-43a0-b12b-55aefc6ce5e2" />
<img width="745" height="362" alt="image" src="https://github.com/user-attachments/assets/cdf04dfc-940b-4f9d-8285-5bacea0be4a6" />
...
<img width="478" height="158" alt="image" src="https://github.com/user-attachments/assets/ac132a43-a43c-4995-b9d4-86f8a07ea3b5" />
...
``bash
 ...
 <img width="1363" height="465" alt="image" src="https://github.com/user-attachments/assets/f25d47e6-23b9-4231-a9c9-ca0af0dbbb41" />
...
``bash
 ...
 <img width="1375" height="577" alt="image" src="https://github.com/user-attachments/assets/11465d27-8c7a-46a2-94e6-40b8428f660b" />
...
``bash
 ...
 <img width="395" height="112" alt="image" src="https://github.com/user-attachments/assets/8f5977d9-33f5-490c-be45-368055f70de8" />
 <img width="1031" height="395" alt="image" src="https://github.com/user-attachments/assets/a924a883-e61d-4ccc-882c-a5d1befbe5c2" />
```
...
```bash
 รูปหน้า Dashboard 
```
<img width="1919" height="1076" alt="image" src="https://github.com/user-attachments/assets/9c4d2299-e535-46a3-8614-d8f8254ae7b9" />
...
```
<img width="1894" height="1052" alt="image" src="https://github.com/user-attachments/assets/dd199443-47c3-4a8d-910f-dd60de2c0a9f" />
...
```
<img width="1887" height="1060" alt="image" src="https://github.com/user-attachments/assets/c658dc7c-cb1e-44b6-a10e-f2026670a43b" />
...
```
<img width="1909" height="1077" alt="image" src="https://github.com/user-attachments/assets/aaa57495-6d4e-49bd-abac-5fe91fefbfb8" />
<img width="1919" height="1075" alt="image" src="https://github.com/user-attachments/assets/0e6447c5-a94b-4ddb-9790-eb7173548083" />
...
```
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/c54f0ee4-0f86-401c-8f43-ab43c78c2e9d" />
...
```
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/1ff426b2-4cd5-4ab1-a7d1-6a7727589c9c" />
...
```
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/a2601e0b-3a4e-4e17-b8d9-b11f39313a40" />
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/234a8f7b-2a98-4bb1-aecb-10b72b8990d3" />
...
คำตอบ:
```
Docker Image คือตัวต้นแบบหรือไฟล์ติดตั้ง (Blueprint) ที่รวมโค้ดและไลบรารีไว้ แก้ไขไม่ได้ (Read-only) เปรียบเหมือน "แผ่นเกม" หรือ "แม่พิมพ์"
Docker Container คือตัวโปรแกรมที่ถูกรันขึ้นมาจาก Image ให้ทำงานได้จริง มีการเปลี่ยนแปลงข้อมูลได้ เปรียบเหมือน "เกมที่เรากำลังเล่นอยู่" หรือ "ขนมที่ปั๊มออกมาจากแม่พิมพ์"
ตัวอย่าง: Image คือ 'moodle:latest' ส่วน Container คือเว็บ Moodle ที่เราล็อกอินเข้าไป
```
...
```
มี 2 ตัว
1. ตัว Web Server (Moodle): ทำหน้าที่รันหน้าเว็บและประมวลผล PHP
2. ตัว Database (db): ทำหน้าที่เก็บข้อมูลทั้งหมดของระบบ เช่น user, password และข้อมูลรายวิชา
```
...
คำตอบ:
```
ใช้แบบ Volume Mapping (Bind Mount) คือการเชื่อมโฟลเดอร์ใน Container ออกมาเก็บที่เครื่องเรา
ข้อดี: ข้อมูลไม่หายเวลาลบหรือ Restart Container
ข้อเสีย: ต้องจัดการ Path โฟลเดอร์ให้ถูก ถ้ากำหนดผิด Container อาจจะหาไฟล์ไม่เจอ หรือมีปัญหาเรื่อง Permission
```
...
คำตอบ:
```
Network สร้างวงแลนเสมือนให้ Container ในโปรเจกต์เดียวกันมองเห็นกันได้
การสื่อสารจะใช้ "ชื่อ Service" (Service Name) แทน IP Address เช่น ตัวเว็บ Moodle สามารถเรียกหาฐานข้อมูลโดยใช้ชื่อ host ว่า "db" ได้
...
คำตอบ:
```
ใช้กำหนดลำดับการรันครับ เพื่อให้มั่นใจว่า Database (db) จะถูกรันขึ้นมาก่อนตัว Moodle ป้องกันไม่ให้ Moodle Error เพราะหาฐานข้อมูลไม่เจอตอนเริ่มระบบ
```
...
คำตอบ:
```
แก้ที่หัวข้อ `ports:` ของ service moodle ครับ
จากเดิม (เช่น 80:80) ให้เปลี่ยนเป็น `9000:80` (เลขหน้าคือ Port เครื่องเรา เลขหลังคือ Port ใน Container)
...
คำตอบ:
```
หมายถึงให้ Moodle วิ่งไปเชื่อมต่อฐานข้อมูลที่เครื่องชื่อ "db"
สาเหตุที่ไม่ใช้ localhost เพราะใน Docker คำว่า localhost หมายถึงตัว Container นั้นๆ เอง ถ้าใส่ localhost ตัว Moodle จะหาฐานข้อมูลในตัวมันเองซึ่งไม่มี
```
...
คำตอบ:
```
ข้อดี Docker: ติดตั้งง่ายมาก คำสั่งเดียวจบ ไม่ต้องมานั่งลง PHP/MySQL เองทีละตัว และไม่ทำให้เครื่องรก
ข้อเสีย Docker: กินทรัพยากรเครื่องมากกว่านิดหน่อย และถ้าจะแก้ Config ลึกๆ อาจจะยุ่งยากกว่าแบบปกติเพราะต้อง Remote เข้าไปแก้ข้างใน
```
...
```
วิธีตรวจสอบ:
1. ใช้คำสั่ง `docker logs <ชื่อ container>` เพื่อดู Error Message ว่าฟ้องอะไร (เช่น Connection refused หรือ Access denied)
2. ใช้คำสั่ง `docker ps` เช็คว่า Container Database รันอยู่ไหม

วิธีแก้ไข:
1. เช็คไฟล์ docker-compose.yml ว่าค่า Environment Variables (User, Password, Database Name) ของทั้งฝั่ง Moodle และ Database ตรงกันหรือไม่
2. ตรวจสอบค่า MOODLE_DB_HOST ว่าใส่ชื่อ Service ถูกต้องหรือไม่ เช่นต้องเป็น db ไม่ใช่ localhost
```
...
```
คำสั่งนี้จะลบทั้ง Container, Network และ **ลบ Volume** ทิ้งไปด้วย ผลคือข้อมูลทุกอย่างที่เคยบันทึกไว้ จะหายเกลี้ยงทั้งหมดเหมือนเพิ่งเริ่มติดตั้งใหม่
---
