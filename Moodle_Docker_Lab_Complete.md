...
คำตอบ:
```
- Image เป็นต้นแบบ เหมือนแผ่นติดตั้งโปรแกรม

- Container เป็นร่างที่ทำงานอยู่ เหมือนการเปิดโปรแกรมขึ้นมาใช้งานจริง
```
...
```
มี 2 ตัว คือ
  1.moodle_app เป็น Web Server ประมวลผล PHP และแสดงผลหน้าเว็บ
  2.db เป็นฐานข้อมูล เก็บข้อมูลดิบและโครงสร้างทั้งหมด
```
...
คำตอบ:
```
ในการทดลองมีการจัดการ Volume แบบ Named Volume ซึ่ง
ข้อดี คือ ข้อมูลไม่หายเมื่อลบ Container, สำรองข้อมูลง่าย
เเละข้อเสีย คือ ต้องจัดการสิทธิ์ไฟล์ให้ถูกต้อง
```
...
คำตอบ:
```
Network ใน Docker Compose ทำหน้าที่สร้างวง LAN จำลองให้ Container คุยกันเองได้ เเละ Container สื่อสารกันโดยใช้ Service Name แทน IP (เช่น พิมพ์ชื่อ db เพื่อเชื่อมต่อ)
```
...
คำตอบ:
```
ใช้กำหนด ลำดับการรัน เพื่อให้ Database เริ่มทำงานเสร็จก่อนที่ App จะเริ่มเชื่อมต่อ
```
...
คำตอบ:
```
Environment Variables `MOODLE_DB_HOST=db` หมายความว่า เป็นการกำหนดว่า Database Server อยู่ที่ชื่อ db
เเละ MOODLE_DB_HOST=db ทำไมไม่ใช้ localhost?เพราะใน Docker แต่ละ Container มี IP แยกกัน localhost
```
...
คำตอบ:
```
- Docker จะติดตั้งเร็ว ย้ายเครื่องง่าย Environment คงที่ แต่กินทรัพยากรเพิ่มเล็กน้อย

- ติดตั้งปกติ จะรันได้เต็มประสิทธิภาพเครื่อง แต่ติดตั้งยาก ย้ายเครื่องลำบาก และคุมเวอร์ชัน Library ยาก
```
...
คำตอบ:
```
วิธีตรวจสอบ: เช็ค Logs ด้วย docker logs moodle_app และดูสถานะด้วย docker ps


วิธีแก้ไข: ตรวจสอบรหัสผ่านในไฟล์ .yml ให้ตรงกัน และเช็คว่าทั้งคู่อยู่ Network วงเดียวกัน
```
...
คำตอบ:
```
Container จะถูกลบและข้อมูลทั้งหมดใน Volume จะหายถาวร
```
