...
<img width="760" height="313" alt="image" src="https://github.com/user-attachments/assets/0a373175-89da-4679-b37b-4044357fd9f6" />
...
**บันทึกผลการทดลอง 2:**
<img width="964" height="225" alt="image" src="https://github.com/user-attachments/assets/40065e0d-0590-4cab-a3e6-b8a2bb083f61" />
...
**บันทึกผลการทดลอง 3:**
 ...
 <img width="1339" height="162" alt="image" src="https://github.com/user-attachments/assets/c53a04ee-2808-4ef3-b3e8-166d699e9b13" />
...
```
**บันทึกผลการทดลอง 4:**
<img width="1325" height="322" alt="image" src="https://github.com/user-attachments/assets/b4dad6ee-9450-451f-be29-db1895dcc3e1" />
...
**บันทึกผลการทดลอง 5:**
<img width="1089" height="523" alt="image" src="https://github.com/user-attachments/assets/599f849c-b372-4bb5-96b7-912a3910f6d7" />
...
**บันทึกผลการทดลอง 6:**
<img width="1864" height="1048" alt="image" src="https://github.com/user-attachments/assets/9d5ee7ab-e53a-4661-b9f1-6a298b99f40a" />
...
<img width="1840" height="1038" alt="image" src="https://github.com/user-attachments/assets/743c76e2-65ca-4f83-ab28-0f79b9c297a9" />
...
**บันทึกผลการทดลอง 7:**
<img width="907" height="935" alt="image" src="https://github.com/user-attachments/assets/859936e8-94b9-4f3f-9428-191559098d16" />
...
**บันทึกผลการทดลอง 8:**
<img width="1864" height="1048" alt="image" src="https://github.com/user-attachments/assets/5f39aec2-e8ab-4373-ab1c-92b903df41ca" />
...
**บันทึกผลการทดลอง 9:**
<img width="1567" height="667" alt="image" src="https://github.com/user-attachments/assets/96ce1965-ab43-4f4c-a854-7cf7a508c0c7" />
...
**บันทึกผลการทดลอง 10:**
<img width="1864" height="1048" alt="image" src="https://github.com/user-attachments/assets/bf7617dd-a297-464c-ad39-d2e7d2cd884c" />
...
คำตอบ:
```
Image คือต้นแบบที่เป็นไฟล์ Read-only (เหมือนแผ่นติดตั้งโปรแกรม) ส่วน Container คือ Instance ที่ถูกรันมาจาก Image นั้น (เหมือนโปรแกรมที่กำลังทำงานอยู่)
```
...
คำตอบ:
```
1. db (Database): ใช้ MariaDB หรือ MySQL เก็บข้อมูลดิบ (เช่น รายชื่อนักเรียน, คะแนน)
2. moodle (App): ตัวประมวลผลเว็บ Moodle และจัดการหน้า UI ที่เราสร้างรายวิชา
```
...
คำตอบ:
```
มักใช้ Named Volumes
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container, จัดการง่ายผ่าน Docker CLI
ข้อเสีย: เข้าถึงไฟล์โดยตรงจาก Host OS ยากกว่าแบบ Bind Mount
```
...
คำตอบ:
```
ทำหน้าที่สร้างเครือข่ายเสมือนให้ Container เชื่อมต่อกัน โดยสื่อสารกันผ่าน Service Name
...
คำตอบ:
```
ใช้กำหนดลำดับการรัน Container เช่น ให้ Database เริ่มทำงานก่อนที่ตัวเว็บ Moodle จะรันขึ้นมา เพื่อป้องกัน Error การเชื่อมต่อฐานข้อมูล
```
...
คำตอบ:
```
แก้ไขที่หัวข้อ ports: ในตัว App โดยเปลี่ยนจาก - 80:8080 เป็น - 9000:8080
...
คำตอบ:
```
หมายถึงให้ Moodle ไปหาฐานข้อมูลที่ชื่อเครื่อง db ใน Network เดียวกัน ที่ไม่ใช้ localhost เพราะในโลกของ Container localhost หมายถึงตัวมันเอง ไม่ใช่เครื่อง Database ที่อยู่แยกกัน
```
...
คำตอบ:
```
ข้อดี: ติดตั้งเร็ว, ย้ายเครื่องง่าย, ไม่กวนระบบหลักในเครื่อง (ไม่ต้องลง PHP/Apache แยกเอง)
ข้อเสีย: กินทรัพยากร RAM มากกว่าเล็กน้อย, ต้องมีความรู้เรื่อง Docker เพิ่มเติม
```
...
คำตอบ:
```
วิธีตรวจสอบ:ใช้คำสั่ง docker logs moodle_app ดู Error หรือ docker ps เพื่อดูว่า Container db รันอยู่ไหม


วิธีแก้ไข:ตรวจสอบรหัสผ่านใน Environment Variables ให้ตรงกันทั้งสองฝั่ง และเช็คว่าชื่อ Host ถูกต้อง
...
คำตอบ:
```
Container จะถูกลบ และ ข้อมูลใน Volume (ฐานข้อมูลและไฟล์ที่อัปโหลด) จะถูกลบถาวร เนื่องจากมี Option -v ต่อท้าย
