...
**บันทึกผลการทดลอง 7:**
<img width="1919" height="870" alt="image" src="https://github.com/user-attachments/assets/533f8f63-c5d2-4a79-b618-69b19c31403b" />
...
**บันทึกผลการทดลอง 8:**
<img width="1919" height="877" alt="image" src="https://github.com/user-attachments/assets/ee4599fd-c9a4-4787-9e68-182cf08a7daa" />
...
**บันทึกผลการทดลอง 9:**
<img width="1919" height="856" alt="image" src="https://github.com/user-attachments/assets/3e4e9dd5-34bd-40c3-a895-e0c4a00f8e95" />
...
**บันทึกผลการทดลอง 10:**
<img width="1919" height="866" alt="image" src="https://github.com/user-attachments/assets/c413c30d-999f-485b-9f9f-67cc48e31395" />
...
5. 
   **บันทึกผลการทดลอง 11:**
<img width="1919" height="826" alt="image" src="https://github.com/user-attachments/assets/c116f3e4-3793-4e12-aad7-3c6c44b09617" />
...
คำตอบ:
- Image: เป็น "ไฟล์ต้นฉบับ" (Read-only) ที่รวมทุกอย่างไว้แต่ยังไม่ทำงาน
- Container: เป็น "ตัวโปรแกรมที่กำลังรัน" ที่สร้างมาจาก Image

**2. จากสถาปัตยกรรมในการทดลอง มี Container กี่ตัว? แต่ละตัวมีหน้าที่อะไร?**

คำตอบ:
มี Container 2 ตัว คือ 
Moodle : เก็บข้อมูลหลักทั้งหมด
db : ประมวลผลหน้าเว็บและให้บริการผู้ใช้งาน
<img width="537" height="321" alt="image" src="https://github.com/user-attachments/assets/f07b1478-a282-45d3-834f-74162c58ab13" />


**3. จากการทดลองมีการจัดการ Volume แบบใด มีข้อดีข้อเสียอย่างไร?**

คำตอบ:
เป็นการจัดการแบบ Named Volume (การตั้งชื่อให้กับพื้นที่เก็บข้อมูล)
- ข้อดี:ข้อมูลไม่สูญหายแม้จะลบหรือหยุดทำงานของ Container แต่ข้อมูลในฐานข้อมูลและไฟล์ที่อัปโหลดไว้จะยังคงอยู่
      จัดการง่าย เพราะ Docker เป็นผู้ดูแลตำแหน่งที่เก็บไฟล์ให้โดยอัตโนมัติ ไม่ต้องกังวลเรื่องการตั้งค่า Permission ของโฟลเดอร์บนเครื่อง Host
- ข้อเสีย: เข้าถึงไฟล์โดยตรงได้ยาก เนื่องจากไฟล์ถูกเก็บไว้ในพื้นที่เฉพาะของ Docker (เช่น /var/lib/docker/volumes/) ทำให้การเข้าไปแก้ไขไฟล์จากภายนอกทำได้ยากกว่าการระบุตำแหน่งโฟลเดอร์เอง

**4. Network ใน Docker Compose ทำหน้าที่อะไร? Container สื่อสารกันอย่างไร?**

คำตอบ:
- ทำหน้าที่ในการสร้างวง LAN จำลองให้ Container คุยกันเองได้
- สื่อสารกันโดยใช้ Service Name (เช่น db, moodle) เป็นชื่อแทน IP Address ทำให้สื่อสารกันได้ทันที


**5. `depends_on` ในไฟล์ docker-compose.yml มีความสำคัญอย่างไร?**

คำตอบ:
ใช้กำหนด "ลำดับการเริ่มต้น" ของ Container เพื่อให้มั่นใจว่า Container ฐานข้อมูล (db) จะถูกเปิดขึ้นมาและพร้อมใช้งานก่อนที่ Container ตัวแอป (moodle) จะเริ่มทำงาน หาก Moodle เริ่มทำงานก่อนที่ฐานข้อมูลจะพร้อม จะทำให้เกิด Error การเชื่อมต่อฐานข้อมูลล้มเหลวและทำให้ระบบล่มตั้งแต่ตอนเริ่มต้น

**6. ถ้าต้องการเปลี่ยน Port ของ Moodle  เป็น 9000 ต้องแก้ไขส่วนใดของไฟล์?**

คำตอบ:
แก้ไขที่หัวข้อ ports: ใน Service ของ moodle โดยเปลี่ยนเลขตัวหน้าเป็น 9000

**7. Environment Variables `MOODLE_DB_HOST=db` หมายความว่าอย่างไร? ทำไมไม่ใช้ `localhost`?**

คำตอบ:
- หมายถึงการระบุให้ Moodle ไปเชื่อมต่อกับ Container ฐานข้อมูลที่ชื่อว่า "db"
- เหตุผลที่ไม่ใช้ localhost: เพราะในระบบ Docker แต่ละ Container จะมองเห็น localhost เป็นแค่ตัวเอง การใช้ชื่อ Service (db) จะทำให้ Docker ช่วยหาเส้นทางไปหา Container ฐานข้อมูลให้โดยอัตโนมัติ


**8. เปรียบเทียบข้อดีและข้อเสียของการติดตั้ง Moodle ด้วย Docker เทียบกับการติดตั้งแบบปกติ**

คำตอบ:
- ข้อดี: ติดตั้งง่ายและเร็วมาก (Command เดียวจบ), ย้ายเครื่องได้สะดวกโดยที่ตั้งค่าทุกอย่างยังเหมือนเดิม (Portable)
- ข้อเสีย: กินทรัพยากรเครื่องมากกว่าเล็กน้อย และหากเกิดปัญหาต้องใช้คำสั่งเฉพาะของ Docker ในการตรวจสอบ
...
คำตอบ:
```
วิธีตรวจสอบ:ใช้คำสั่ง docker-compose ps เพื่อดูว่าฐานข้อมูล Status เป็น Up หรือไม่ และใช้ docker-compose logs db เพื่อดูข้อความแจ้งเตือนข้อผิดพลาด


วิธีแก้ไข: ตรวจสอบชื่อ DB_USER และ DB_PASSWORD ในไฟล์ให้ตรงกันทั้งสองฝั่ง และเช็กว่าทั้งคู่ต่ออยู่ใน networks เดียวกันหรือไม่
...
คำตอบ:
```
ข้อมูลทั้งหมดจะ "ถูกลบออกอย่างถาวร"
