...
**บันทึกผลการทดลอง 1:**
 ...
 <img width="888" height="626" alt="image" src="https://github.com/user-attachments/assets/047a0147-3232-4c60-870b-b09aee4d487b" />
...
**บันทึกผลการทดลอง 2:**
 ...
 <img width="879" height="217" alt="image" src="https://github.com/user-attachments/assets/de0deca9-1717-4783-94a9-48175268f508" />
...
**บันทึกผลการทดลอง 3:**
 ...
 <img width="882" height="180" alt="image" src="https://github.com/user-attachments/assets/a71a57e6-e19c-424b-b0d2-cef83d7cfea0" />
...
```
**บันทึกผลการทดลอง 4:**
<img width="894" height="619" alt="image" src="https://github.com/user-attachments/assets/55a1a8f5-32e4-44e1-b3ee-21824bf5dbcb" />
...
**บันทึกผลการทดลอง 5:**
 ...
 <img width="893" height="708" alt="image" src="https://github.com/user-attachments/assets/5cbf6420-6e2f-472b-a8aa-92997e5ba0ab" />
...
```
 ...
 <img width="1910" height="963" alt="image" src="https://github.com/user-attachments/assets/9f35ce2d-4c56-4427-80b4-32adb33018b3" />
...
**บันทึกผลการทดลอง 6-1:**
 ...
 <img width="1919" height="1017" alt="image" src="https://github.com/user-attachments/assets/d9da7487-6de0-474a-8985-b43eef46e0b5" />
...
```
 ...
 <img width="941" height="985" alt="image" src="https://github.com/user-attachments/assets/5214b4de-d88b-43c9-a4c6-7b2b12fea325" />
...
**บันทึกผลการทดลอง 8:**
```
<img width="1919" height="1018" alt="image" src="https://github.com/user-attachments/assets/b1a80afa-aaba-4231-a456-156c68ca95f4" />
...
**บันทึกผลการทดลอง 9:**
```
<img width="950" height="861" alt="image" src="https://github.com/user-attachments/assets/4922a7df-360d-488f-9ca5-0927273e135d" />
...
<img width="1905" height="867" alt="image" src="https://github.com/user-attachments/assets/a60f4628-7caa-4a43-a149-eb56b8c755d5" />
...
5. 
   **บันทึกผลการทดลอง 11:**
```
 <img width="957" height="872" alt="image" src="https://github.com/user-attachments/assets/8cbe17b1-f1f4-4a7d-8992-55be6bbbbcb2" />

 <img width="1919" height="860" alt="image" src="https://github.com/user-attachments/assets/03e5245c-a217-4995-9990-dae76233783c" />
...
คำตอบ: Docker Image คือ แม่พิมพ์ หรือ แปลนบ้าน ที่มีการแพ็คโปรแกรมและ Library ทุกอย่างไว้แล้ว เป็นแบบ Read-only (แก้ไขไม่ได้)

Docker Container คือ สิ่งก่อสร้างที่ใช้งานจริง ที่ถูกสร้างมาจาก Image เป็น Process ที่กำลังทำงานอยู่ (Running state) และสามารถเขียนข้อมูลลงไปได้

ตัวอย่าง : lthub/moodle:education-4.5.8 คือ Image (ไฟล์ต้นฉบับ) ส่วน moodle_app ที่เราสั่ง Start ขึ้นมาใช้งานคือ Container
```
...
คำตอบ: มีทั้งหมด 2 ตัว คือ

db (mariadb): ทำหน้าที่เป็น Database Server เก็บข้อมูลทั้งหมดของระบบ Moodle (เช่น รายชื่อผู้ใช้, คะแนน, รายวิชา)

moodle (lthub/moodle): ทำหน้าที่เป็น Web Application Server ประมวลผล PHP และแสดงหน้าเว็บ Moodle ให้ผู้ใช้เห็น
```
...
คำตอบ: ใช้การจัดการแบบ Named Volumes (เช่น db_data และ moodledata)

ข้อดี ข้อมูลมีความคงทน (Persistent) ไม่หายไปพร้อมกับ Container, Docker เป็นคนจัดการพื้นที่จัดเก็บให้, แชร์ข้อมูลระหว่าง Container ได้ง่าย

ข้อเสีย การเข้าถึงไฟล์เหล่านี้โดยตรงจาก Host (Windows/Mac) ทำได้ยากกว่าแบบ Bind Mount (ที่ map folder ออกมาตรงๆ)
```
...
คำตอบ: ทำหน้าที่สร้าง Virtual Network (วง LAN จำลอง) เพื่อเชื่อมต่อ Container เข้าด้วยกันอย่างปลอดภัย แยกออกจาก Network ภายนอก

การสื่อสาร: Container จะสื่อสารกันผ่าน Service Name (ชื่อบริการ) ระบบ Docker จะมี Internal DNS ที่คอยแปลงชื่อ เช่น เมื่อ Moodle เรียกหา db มันจะรู้ทันทีว่าต้องวิ่งไปที่ IP Address ไหน
```
...
คำตอบ: ใช้กำหนด ลำดับการเริ่มต้นทำงาน (Start Order) เพื่อให้แน่ใจว่า Container ที่เป็นฐานข้อมูล (db) ถูกสั่งให้เริ่มทำงานก่อน Container ที่เป็นแอปพลิเคชัน (moodle) เพื่อลดโอกาสเกิด Error จากการหาฐานข้อมูลไม่เจอ
```
...
คำตอบ:แก้ไขในส่วน services: > moodle: > ports

จากเดิม: - "80:80"

เปลี่ยนเป็น: - "9000:80" (ซ้ายคือ Port เครื่องเรา ขวาคือ Port ใน Container)
```
...
คำตอบ: หมายถึงการบอกให้ Moodle เชื่อมต่อฐานข้อมูลที่เครื่องชื่อว่า db (ซึ่งเป็น Service Name ของ MariaDB)

ทำไมไม่ใช้ localhost: เพราะในโลกของ Container คำว่า localhost (127.0.0.1) หมายถึง "ตัว Container Moodle เอง" ซึ่งไม่ได้มีฐานข้อมูลติดตั้งอยู่ ฐานข้อมูลอยู่อีก Container นึง จึงต้องเรียกชื่อให้ถูกตัว
```
...
คำตอบ: แบบ Docker (ข้อดี) ติดตั้งง่าย (คำสั่งเดียว), สภาพแวดล้อมเหมือนกันทุกเครื่อง (ไม่เจอปัญหา "เครื่องฉันรันได้ เครื่องเธอรันไม่ได้"), ลบหรืออัปเกรดง่ายโดยไม่กระทบ OS หลัก

แบบปกติ (XAMPP/Manual) (ข้อดี) แก้ไขไฟล์ Config ได้โดยตรงง่ายกว่า (ข้อเสีย) ติดตั้งยุ่งยาก, ต้องจัดการ Version ของ PHP/MySQL เอง, อาจเกิด Conflict กับโปรแกรมอื่นในเครื่อง
```
...
คำตอบ (เขียน YAML): เพิ่ม Service redis ต่อท้าย moodle และ db
services:
  # ... (service อื่นๆ)
  redis:
    image: redis:alpine
...
คำตอบ:
```
วิธีตรวจสอบ: ใช้คำสั่ง docker-compose logs moodle เพื่อดู Error Message หรือ docker-compose ps เพื่อดูว่า Database รันอยู่หรือไม่ (State Up/Exit)

วิธีแก้ไข: ตรวจสอบชื่อตัวแปร MYSQL_PASSWORD กับ MOODLE_DB_PASSWORD ในไฟล์ docker-compose.yml ว่าตรงกันไหม

ตรวจสอบว่า MOODLE_DB_HOST ใส่ชื่อถูกต้องหรือไม่ (db)

ถ้าแก้แล้ว ให้รัน docker-compose down -v เพื่อล้างค่าเก่า แล้ว up ใหม่

```

**11. ถ้ารัน `docker-compose down -v` จะเกิดอะไรขึ้นกับข้อมูล?**

คำตอบ: คำสั่งนี้จะลบ Container, Network และที่สำคัญคือ ลบ Volumes ทั้งหมดทิ้ง (-v) ส่งผลให้ ข้อมูลทุกอย่าง (รายชื่อผู้ใช้, ข้อมูลในเว็บ, ฐานข้อมูล) จะสูญหายถาวร เหมือนล้างเครื่องใหม่ครับ
```
