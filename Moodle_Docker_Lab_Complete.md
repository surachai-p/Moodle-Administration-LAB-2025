...
**บันทึกผลการทดลอง 1:**
``bash
 <img width="853" height="337" alt="image" src="https://github.com/user-attachments/assets/e3d3fb6c-3110-4af3-8264-542f14c61730" />
 <img width="810" height="324" alt="image" src="https://github.com/user-attachments/assets/36d401e5-5051-40d3-8330-7667016c9d10" />
...
<img width="1209" height="347" alt="image" src="https://github.com/user-attachments/assets/10f3e606-7c11-4429-825a-08050423e563" />
...
<img width="1095" height="264" alt="image" src="https://github.com/user-attachments/assets/511b00aa-2471-4f2c-bff4-fbe77a3a9c16" />
...
``bash
 ...
 <img width="1092" height="680" alt="image" src="https://github.com/user-attachments/assets/261b593f-b0bd-4fbc-a446-32bd4c683050" />
...
<img width="1071" height="513" alt="image" src="https://github.com/user-attachments/assets/581480cf-ea62-46f1-a7e7-5c8ef7f2580b" />
...
``bash
 รูปหน้า Dashboard
<img width="1919" height="1060" alt="image" src="https://github.com/user-attachments/assets/9b6558c1-2d7f-4cc5-9332-92fc6d700a77" />
...
``bash
 ...
 <img width="1919" height="1076" alt="image" src="https://github.com/user-attachments/assets/15a80298-fc5c-46cf-8758-75f3727889ba" />
...
``bash
 ...
 <img width="736" height="595" alt="image" src="https://github.com/user-attachments/assets/6bdbda3c-294d-45ec-9b63-9b40ed796ebe" />
``
...
<img width="1919" height="869" alt="image" src="https://github.com/user-attachments/assets/dc845c57-f2cf-45ff-907b-857783cb3819" />
...
คำตอบ:
```
Docker Image: เหมือน "สูตรอาหาร" หรือแผ่นติดตั้งโปรแกรม (ISO) ที่แก้ไขไม่ได้ เป็นต้นแบบนิ่งๆ

Docker Container: เหมือน "อาหารที่ปรุงเสร็จแล้ว" คือการเอา Image มาสั่งให้ทำงานจริง เป็น Runtime ที่เราเข้าไปใช้งานได้

ตัวอย่าง: Image คือไฟล์ตัวติดตั้ง Windows ส่วน Container คือคอมพิวเตอร์ที่กำลังรัน Windows เครื่องนั้นอยู่
...
คำตอบ:
```
Moodle Container: ตัวประมวลผลเว็บ เขียนโปรแกรม จัดการหน้าจอ (Web Application)

Database Container (เช่น MariaDB/MySQL): ตัวเก็บข้อมูลดิบทั้งหมด เช่น รายชื่อนักเรียน คะแนนสอบ
```
...
คำตอบ:
```
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container (Persistence) และแชร์ข้อมูลระหว่างเครื่อง Host กับ Container ได้ง่าย

ข้อเสีย: ถ้าจัดการไม่ดีอาจมีปัญหาเรื่อง Permission (สิทธิ์การเขียนไฟล์) หรือกินพื้นที่ดิสก์โดยไม่รู้ตัวถ้าลืมลบ
```
...
คำตอบ:
```
หน้าที่: สร้าง "วงแลนเสมือน" ให้ Container มาเชื่อมต่อกัน

การสื่อสาร: Container คุยกันผ่าน Service Name (เช่น Moodle พิมพ์หา db ได้เลย) โดย Docker มีระบบ DNS ภายในจัดการให้
...
คำตอบ:
```
ใช้กำหนด ลำดับการเริ่มทำงาน เช่น บอกว่า "ให้ Database รันให้เสร็จก่อนนะ แล้วค่อยเปิด Moodle ตามมา" เพื่อป้องกัน Moodle พังเพราะหา Database ไม่เจอตอนเริ่มต้น
```
...
```
ports:
  - "9000:80"  # เปลี่ยนเลขตัวหน้าจาก 80 หรือ 8080 เป็น 9000
...
คำตอบ:
```
ความหมาย: บอก Moodle ว่าฐานข้อมูลอยู่ที่ Container ชื่อ db

ทำไมไม่ใช้ localhost: เพราะในโลกของ Docker คำว่า localhost ของ Moodle หมายถึงตัวมันเอง ซึ่งไม่มีฐานข้อมูลอยู่ในนั้น จึงต้องเรียกชื่อ Container แทน
```
...
คำตอบ:
```
หัวข้อDockerติดตั้งปกติ (Manual)ความง่ายลงเสร็จในคำสั่งเดียวต้องไล่ลง PHP, Apache, DB ทีละตัวความสะอาดลบง่าย ไม่รกเครื่องลบยาก มักมีไฟล์ค้างประสิทธิภาพกินทรัพยากรน้อยมาก (ใกล้เคียงปกติ)เร็วที่สุดเพราะไม่มี Layer เสริม
```
...
```
วิธีตรวจสอบ:
วิธีตรวจสอบ: ใช้คำสั่ง docker logs [ชื่อ container db] ดูว่า Error อะไร หรือ docker ps ดูว่า DB รันอยู่ไหม

วิธีแก้ไข: เช็คชื่อ User/Password ในไฟล์ .env หรือ yml ให้ตรงกัน และดูว่าทั้งคู่ต่อ Network วงเดียวกันหรือเปล่า
...
คำตอบ:
```
ข้อมูลหายเกลี้ยงครับ" เพราะ Flag -v (Volumes) จะสั่งให้ Docker ลบถังเก็บข้อมูลที่ผูกไว้กับโปรเจกต์นี้ทิ้งไปด้วย (เหมือนล้างเครื่องใหม่เอี่ยม)
