...
คำตอบ: ำ
```
Docker Image: เปรียบเสมือน "พิมพ์เขียว" หรือแผ่นซีดีติดตั้งโปรแกรม เป็นไฟล์ที่อ่านได้อย่างเดียว (Read-only) ที่รวมทุกอย่างที่โปรแกรมต้องใช้ไว้

Docker Container: คือ "ร่างจำลอง" ที่ทำงานอยู่จริงจากการรัน Image เปรียบได้กับโปรแกรมที่กำลังทำงานอยู่ในหน่วยความจ
```
...
คำตอบ:
```
แบบ Named Volumes: (เช่น moodle_data, mariadb_data)
...
คำตอบ:
```
ทำหน้าที่: สร้างวงเครือข่ายเสมือนเพื่อให้ Container คุยกันเองได้
...
คำตอบ:
```
ใช้กำหนดลำดับการรัน Container เช่น ให้ db เริ่มทำงานและพร้อมใช้งานก่อนที่ moodle จะเริ่มทำงาน เพื่อป้องกัน Error จากการหาฐานข้อมูลไม่เจอ
```
...
คำตอบ:
```
แก้ไขที่ส่วน ports: ใน moodle_app จากเดิม 80:8080 เป็น 9000:8080
...
คำตอบ: 
```
ให้ Moodle เชื่อมต่อไปยังเครื่องที่ชื่อว่า db (ซึ่งคือ Container ฐานข้อมูล)
...
คำตอบ: 
```
ข้อดี: ติดตั้งเร็วมาก (คลิกเดียวจบ), ย้ายเครื่องง่าย (Portable), เวอร์ชั่นของ PHP/DB ไม่ตีกับงานอื่นในเครื่อง
...
```
วิธีตรวจสอบ:ใช้คำสั่ง docker-compose logs moodle_app เพื่อดู Error หรือเช็คสถานะด้วย docker ps
