...
```
<img width="685" height="254" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 14 36 33" src="https://github.com/user-attachments/assets/25d298fe-c90a-49e9-b29f-8b20ac30c235" />
...
```
<img width="352" height="92" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 14 38 32" src="https://github.com/user-attachments/assets/f201ad0d-a320-40ab-860b-2f3fcfe3e1fc" />
...
```
<img width="1056" height="73" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 14 39 37" src="https://github.com/user-attachments/assets/978c68c7-0fa0-4dd2-bd54-2790a332e8fe" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 14 52 54" src="https://github.com/user-attachments/assets/3591b1f4-b06c-485b-b08c-688ac792d8a0" />
...
```
<img width="817" height="438" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 14 54 29" src="https://github.com/user-attachments/assets/ab2fa9e5-a5e5-46fc-a0be-1db068d26afc" />
...
```bash
 รูปหน้า Dashboard 
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 06 59" src="https://github.com/user-attachments/assets/1df4de30-1834-4f38-b87b-5b8338245060" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 22 01" src="https://github.com/user-attachments/assets/a48d94c1-ad07-4e07-8d6c-7c93739cc363" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 30 06" src="https://github.com/user-attachments/assets/3ff76609-1069-4b6b-a380-d746241e4b79" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 38 20" src="https://github.com/user-attachments/assets/6a1466a1-65df-41f2-95a4-afb6e701673d" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 43 37" src="https://github.com/user-attachments/assets/6779a4a8-ebd5-4773-a061-7c0029a9ccf4" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 15 50 03" src="https://github.com/user-attachments/assets/bf5e3143-835b-47d2-8834-7594f47de4b3" />
...
```
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 16 09 04" src="https://github.com/user-attachments/assets/e58851cd-0fc3-42af-bc24-8bd47dddbd2f" />
<img width="1440" height="900" alt="ภาพถ่ายหน้าจอ 2569-02-06 เวลา 16 09 25" src="https://github.com/user-attachments/assets/4ed2c3ce-349e-4d48-8c1e-24a50642d97d" />
...
คำตอบ:
```
Docker Image: เป็น "พิมพ์เขียว" (Blueprint) หรือไฟล์ Read-only ที่บรรจุโปรแกรม, Library และการตั้งค่าต่าง ๆ ไว้ข้างใน ไม่สามารถเปลี่ยนแปลงได้
Docker Container: เป็น "ร่างจำลอง" (Instance) ที่รันมาจาก Image เปรียบเสมือนบ้านที่สร้างเสร็จแล้วและใช้งานได้จริง
ตัวอย่าง: เปรียบ Image เหมือน แผ่น DVD เกม (ตัวต้นฉบับ) ส่วน Container คือ การลงเกมในเครื่องแล้วเปิดเล่น (ตัวที่ทำงานอยู่)
```
...
คำตอบ:
```
1. db (MariaDB/MySQL): ทำหน้าที่จัดเก็บฐานข้อมูลทั้งหมดของ Moodle (เช่น ชื่อผู้ใช้, คะแนนสอบ)
2. moodle (Moodle App): ทำหน้าที่ประมวลผล PHP และแสดงผลหน้าเว็บไซต์ (Application Logic)
```
...
คำตอบ:
```
ในการทดลองส่วนใหญ่ใช้ Named Volumes (เช่น moodle_data, mariadb_data)
ข้อดี: ข้อมูลไม่หายเมื่อลบ Container, Docker จัดการตำแหน่งที่เก็บไฟล์ให้เอง, ย้ายข้อมูลข้ามเครื่องได้ง่าย
ข้อเสีย: เข้าถึงไฟล์จากเครื่อง Host โดยตรงได้ยากกว่า Bind Mount (ต้องเข้าไปใน Path เฉพาะของ Docker)
```
...
คำตอบ:
```
หน้าที่: สร้างวง LAN จำลอง (Isolated Network) เพื่อให้ Container คุยกันได้
การสื่อสาร: Container สื่อสารกันผ่าน Service Name (เช่น Moodle จะมองหา DB ผ่านชื่อ "db" แทน IP)
```
...
คำตอบ:
```
ใช้กำหนด ลำดับการเริ่มทำงาน (Startup Order) เพื่อบอกว่า Container หนึ่งต้องรอให้อีกตัวเริ่มทำงานก่อน เช่น moodle ต้องรอให้ db รันขึ้นมาก่อน ไม่อย่างนั้น App จะพยายามเชื่อมต่อฐานข้อมูลที่ยังไม่พร้อมจนเกิด Error
```
...
```
ports:
  - "9000:8080"  # เปลี่ยนเลขหน้าเครื่องหมาย : (Host Port)
```
...
คำตอบ:
```
ความหมาย: บอกให้ Moodle ไปเชื่อมต่อฐานข้อมูลที่ Container ชื่อ db
เหตุผล: ในโลกของ Docker คำว่า localhost ของ Container Moodle หมายถึง "ตัวมันเอง" ซึ่งข้างในไม่มีฐานข้อมูลอยู่ จึงต้องระบุชื่อ Service เพื่อให้ Docker Network ช่วยนำทางไปยัง Container ที่รันฐานข้อมูลอยู่จริง ๆ
```
...
คำตอบ:
| หัวข้อการเปรียบเทียบ | การติดตั้งด้วย Docker | การติดตั้งแบบปกติ (Bare Metal) |
| :--- | :--- | :--- |
| **ความง่ายในการติดตั้ง** | **รวดเร็วมาก** ใช้เพียงคำสั่งเดียว ระบบจัดการ Dependencies และ Environment ให้เสร็จสรรพ | **ยุ่งยาก** ต้องไล่ติดตั้งและตั้งค่า Apache/Nginx, PHP, MariaDB และ Extension ต่างๆ ด้วยตัวเอง |
| **ความเป็นระเบียบ** | **แยกส่วนชัดเจน** ทุกอย่างอยู่ใน Container เมื่อลบทิ้งระบบในเครื่อง Host จะยังคงสะอาดเหมือนเดิม | **กระจัดกระจาย** ไฟล์คอนฟิกและ Library ต่างๆ จะถูกติดตั้งลงใน System Path ของเครื่องโดยตรง |
| **ความสม่ำเสมอ (Consistency)** | **สูงมาก** ทุกคนในทีมใช้ Environment เดียวกัน 100% ลดปัญหา "Works on my machine" | **ต่ำ** มักเกิดปัญหาจากความต่างของเวอร์ชัน OS หรือซอฟต์แวร์ที่ติดตั้งไว้ก่อนหน้า |
| **ประสิทธิภาพ (Performance)** | **มี Overhead เล็กน้อย** จากการจัดการ Resource ของ Docker Layer แต่แทบไม่ส่งผลในการใช้งานทั่วไป | **รันได้เต็มประสิทธิภาพ** เนื่องจากโปรแกรมเข้าถึงทรัพยากรของ Hardware ได้โดยตรงโดยไม่มีเลเยอร์กั้น |
| **การสำรองข้อมูล/ย้ายระบบ** | **ทำได้ง่าย** เพียงแค่ก๊อปปี้ไฟล์ YAML และ Volume ไปรันที่เครื่องใหม่ได้ทันที | **ทำได้ยาก** ต้องสำรองทั้งไฟล์เว็บและฐานข้อมูล รวมถึงต้องเซ็ต Environment เครื่องใหม่ให้ตรงกัน |
...
คำตอบ:
```
วิธีตรวจสอบ: ใช้คำสั่ง docker-compose logs moodle เพื่อดู Error Message และ docker ps เพื่อดูว่า Container db รันอยู่หรือไม่
วิธีแก้ไข: ตรวจสอบ Username/Password ในไฟล์ .yml ว่าตรงกันไหม, เช็คว่า db รันสำเร็จหรือไม่, หรือรอให้ db เริ่มระบบเสร็จก่อนค่อย Restart moodle
```
...
คำตอบ:
```
คำสั่งนี้จะทำการ ลบ Container พร้อมกับลบ Volumes ทั้งหมดทิ้ง ส่งผลให้ข้อมูลในฐานข้อมูลและไฟล์ที่อัปโหลดไว้ใน Moodle หายไปถาวร (เหมือนการ Factory Reset ใหม่ทั้งหมด)
```
